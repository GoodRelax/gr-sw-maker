...
> **言語ツリーが 1 つしか無い場合、Phase 0 を丸ごと飛ばす**（2026-08-12 追加）。`framework-src/` 直下の言語ディレクトリを数え、**2 つ未満なら「比較相手なし」と記録して Phase 1 へ進む。** 現在は `ja` のみであり、**この場合に Phase 0 を実行すると存在しないファイルを開こうとして必ず止まる。**
>
> **同じ理由で、Phase 1 以降のレビュー対象も実在する言語ツリーから引く。** 下表は `framework-src/en/` を前提に書かれているが、**`en` が無いときは同じ相対パスを `framework-src/ja/` に読み替える。**
...
Phase 0 を通過した場合、日英の内容は一致しているため、以降は **EN 版のみ** をレビューする。**Phase 0 を飛ばした場合（言語ツリーが 1 つ）は、下表のパスを実在する言語ツリーへ読み替えてレビューする。**
...
4. クロスチェック結果（**Expert 4 のクロスチェック対象表に定義された各ペアについて** PASS/FAIL）
5. 統合指摘一覧
6. 総合判定
