...
3. **ゲート判定の場合:**
   ...
   - 3d. 同一ゲートの FAIL が上限回数に達した場合、waiver 判断またはユーザーへのエスカレーションを決定する（**簡易・標準は 3 回目、厳格は 2 回目**。後述「ゲート再試行ポリシー」）
...
| FAIL 回数 | 簡易 / 標準 | 厳格 |
|:--------:|------|------|
| 1 回目 | 戻し先を決定し、修正を要請する | 同左 |
| 2 回目 | 同上 | **ユーザーへのエスカレーションを要請する。waiver の要否を判断する** |
| 3 回目 | ユーザーへのエスカレーションを要請する。waiver の要否を判断する | — |

**厳格は 1 段早く上げる。** 開発方式は依頼文の与件で渡される（プロセス規則 §9.1.1）。
