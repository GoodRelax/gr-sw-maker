...
- [ ] GPL/AGPL ライブラリが含まれる場合、orchestrator に報告済み
...
| CLAUDE.md | orchestrator (setup) | ライセンスポリシーの確認 | ライセンスポリシーの記載 |
...
7. 問題のあるライセンスが見つかった場合、orchestrator に報告する
...
| GPL v2/v3 | 要確認 | 必要 | あり | orchestrator に報告 |
| AGPL | 要確認 | 必要 | あり（ネットワーク経由含む） | orchestrator に報告 |
| 不明 | — | — | — | orchestrator に確認を求める |
...
| 依存定義ファイルが存在しない | 作業を開始しない。orchestrator に報告 |
| ライセンス情報が取得できないライブラリがある | 不明ライセンスとして記録し、orchestrator にユーザー確認を求める |
| GPL/AGPL ライブラリが検出された | 即座に orchestrator に報告。利用可否をユーザーに確認 |
