...
- **`gate_phase` を必ず書く（MUST）。** `tools/gate-guard.mjs` はこの欄でゲートを絞ってから観点を集めるため、**欄が無い review、および下表に無い値の review は、どのゲートの証拠にもならない。**

| 手順 | ゲート | `gate_phase` に書く値 |
|:-:|---|---|
| `2i` | GATE-PLANNING | `planning->dependency-selection`。**外部依存の 3 フラグがすべて不成立で Phase 3 を飛ばす場合は `planning->design`** |
| `4m` | GATE-DESIGN | `design->implementation` |
| `5e` | GATE-IMPL | `implementation->testing` |
| `6k` | GATE-TEST | `testing->delivery` |
| `7a` | GATE-DELIVERY | **`gate_phase` ではなく `gate: GATE-DELIVERY` を書く**（納品ゲートは書込みを守っていないため） |

- **半角矢印・空白なしで書く。** プロセス規則 §9.4.1 の遷移列は全角矢印と空白で印字されているが、**そのまま写してはならない（MUST NOT）** —— 機械照合が外れる。**番号で書いてもならない（MUST NOT）**
- **ゲートは観点を共有する**（GATE-IMPL の 5 観点のうち 4 つは GATE-DESIGN と同じ）。`gate_phase` が無いと、**設計レビューの証拠で実装ゲートが開いてしまう**
...
**合格線は開発方式で変わる。** 方式は依頼文の与件で渡される。正本はレビュー観点規約「重大度別の対応ルール」であり、下表はその写しである。

| 重大度 | 簡易 | 標準 | 厳格 |
|---|---|---|---|
| Critical | **0 件**（必須） | 同左 | 同左 |
| High | **0 件**（必須） | 同左 | 同左 |
| Medium | 全件に対応記録があればよい | 同左 | **0 件（修正済みのみ）** |
| Low | **記録不要** | 全件に対応記録があればよい | 同左 |

**再レビューの範囲:** 標準は「修正済み」とした指摘のみ、**厳格は全指摘**（据置き・受容も検証する）。
