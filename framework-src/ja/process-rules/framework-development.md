...
`maintenance-tools/check-parity.mjs` が構造の一致を検査し、pre-commit hook が片側だけのコミットを拒否する。
...
`maintenance-tools/check-tagnames.mjs` がこの突合を行う。

### 5.3 参照先の節が実在することを確認する

エージェント定義の「読むべき規則の節」に節番号を書いた場合、その節が実在することを確認する。存在しない節を指すと、エージェントは代わりに規則全文を読む。

`maintenance-tools/check-links.mjs` が、Markdown リンクと節番号参照の双方の実在を検査する。**節番号は実在するが題名が違う**場合は検出できない。これは目視で確認する。
...
規則・フィールド・検査・エージェントを**増やす**提案には、次の 3 点を併記しなければならない（MUST）。

| # | 併記する内容 |
|:-:|---|
| 1 | **追加しない場合に何が起きるか。** 観測された事象があれば件数を添える |
| 2 | **より小さい代替案**（最小構成） |
| 3 | **最小構成を採らなかった理由** |
...
### 5.6 既定値と MUST には理由を併記する（MUST・範囲限定）

**対象は既定値と MUST に限る。** 全条文に理由を書けば記述量が増え、5.5 が防ごうとしている肥大を規則の側から招く。

| 対象 | 併記する内容 |
|---|---|
| **既定値** | なぜその既定なのか。**変える場合にどこへ記録するか** |
| **MUST** | 守らなかったときに何が壊れるか |

**なぜ:** 本試行では CLAUDE.md テンプレートのサーバー / Web 前提の既定値が CLI と **6 箇所**で衝突し、その都度 decision による読み替えを要した。**「なぜその既定なのか」が書かれていれば、CLI には当てはまらないと即座に判断できた。** 読む主体は LLM であり、既定しか書かれていなければ既定に従う。

**最小構成との比較（5.5 の自己適用）:**

| # | 内容 |
|:-:|---|
| 1 | **追加しない場合:** 既定値の読み替えが再発する。本試行での実績は 6 箇所 |
| 2 | **最小構成:** 既定値にのみ理由を書き、MUST は対象外とする |
| 3 | **採らなかった理由:** 本試行で読み替えを要したのは既定値だが、**争点になったのはいずれも MUST であった。** MUST を外すと、なぜその義務があるのかを毎回議論し直すことになる |
