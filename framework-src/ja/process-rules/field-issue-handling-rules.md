...
- ユーザーフィードバックを仕様書と照合し、defect か CR（仕様変更）かを正確に分類する
...
- 自動テストで検出される defect は対象外（従来の defect プロセスを使用する）
...
### 6.0 3 つの終端とゲート不成立時の戻り先

field-issue は `verified` だけで終わるわけではない。**3 つの終端がある。**

| 終端 | 条件 | メトリクス集計 |
|------|------|---------------|
| `verified` | 修正され、ユーザーが実機で確認した | 対象 |
| `rejected` | 仕様どおりの動作であり defect ではないと判定した | 対象外 |
| `cannot-reproduce` | 再現試行を尽くしたが再現しなかった | 対象外 |

**`cannot-reproduce` にするには再現試行レポート 6 項目が必須である（MUST）。** 試行回数・環境・入力条件・元の報告との差分・確認手段・再オープン条件。1 つでも欠ければ終端にできず `reported` のまま残す。
...
| 不成立のゲート | 戻り先 | 理由 |
|---------------|--------|------|
| §6.1 分類が確定しない | `reported`（field-test-engineer） | 情報が足りない。追加のログ・再現手順を集める |
| §6.3 根本原因が特定できない | `in-analysis` に留まる | 推測で `cause-identified` に進めない |
| §6.5 対策案を 1 つに絞れない | `in-planning` に留まる | 代替案を全て列挙し、ユーザーに方針を仰ぐ |
| §6.8 仕様レビューが FAIL | `spec-updated`（srs-writer / architect） | 仕様の修正が不十分 |
