...
| 3.3     | File Structure       | ファイル構成         | 常に     | ディレクトリ構成。コンポーネントとフォルダの対応                         |
...
| 3.6     | Decisions            | 設計判断             | 常に     | ADR（Architecture Decision Records）。判断理由・代替案・決定者。記録形式は Michael Nygard の ADR フォーマット（Status / Context / Decision / Consequences）を推奨。**ADR-000「最小構成との比較」を必ず含める**（要求を満たす最小の構成、採用案が増やした要素、各々を増やした理由。R2.18） |
