...
開発方式を 1 つ決めれば、仕様書・作業・担当者・成果物・レビューがすべて決まる。**Phase 1 の `1e` で判定し、`CLAUDE.md`「開発方式」節に記録する（MUST）。**
...
**どの手順を実施し、どれを免除するかは `process-rules/development-mode.md` の表 M が持つ（97 手順ぶん）。本節に再掲しない。**

**本節が持つのは判定基準だけである。** 免除の粒度は手順であって「プロセス / 成果物」ではない。**2 か所で決めると必ずずれる。**
...
**計測経路が働かない場合がある。** 受け口が起動していなければ、テレメトリは受け取られずに黙って捨てられる。**受け口の起動は `/full-auto-dev` の Phase 0 で行う**（Windows は `tools/start-otel-sink.bat`、他の OS は `node tools/otel-sink.mjs`）。したがって**存在だけでなく鮮度を検査する。** `session-state.json` の `sink_heartbeat_at` は受け口が生きている限りトラフィックの有無によらず更新されるため、これが現在時刻から大きく離れていれば受け口は停止している。この場合、**欠測であることを cost-log.json とフェーズ報告に明記する。消費量を推測で埋めてはならない（MUST NOT）。**計測不能のまま黙って進むと、コスト超過は検知されないまま進行する。
...
**合格線は開発方式で変わる**（レビュー観点規約「重大度別の対応ルール」が正本。2026-08-12 に本節へ反映）。**方式は依頼文の与件で渡される。**
