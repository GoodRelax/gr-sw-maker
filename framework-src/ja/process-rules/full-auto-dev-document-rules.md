...
**改訂は差分の適用だけでは終わらない（MUST）。** 変えた内容に従属する既存記述が古くなっていないかを確認し、結果を change_log に記録する。

`change_log` は何を変えたかを記録するが、**変えたことで従属記述が古くなったかは誰も見ない。** 本試行では、ふりかえりがこのパターンを構造的と判定した後、規則を手当てしなかったために design で同一パターンが新たに 7 件発生した。

**確認する記述の種類:**

| 種類 | 本試行での件数 | 見落とすと何が起きるか |
|---|:-:|---|
| 図（クラス図・シーケンス図・状態遷移図・分岐図） | **4** | 本文と図が食い違い、どちらが正か読み手が判断できない |
| ADR の Consequences | 1 | 決定の帰結が古い前提のまま残る |
| 本文中の「影響」を述べた行 | 1 | 影響範囲の記述が実態より狭くなる |
| コード内の識別子 | 1 | 仕様の名前と実装の名前が乖離する |

**記録の仕方:** 該当する種類について確認したことと結果を `action` 列に書く（例: `"updated Ch3.2; checked diagrams (2 updated), ADR consequences (no change)"`）。

**この規約の限界:** 自己申告であり、**列挙にない種類は漏れる。** それでも、対象が列挙されていれば「確認していない」ことが可視になる。網羅性ではなく可視性のための規約である。
