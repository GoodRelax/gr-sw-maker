...
6. **エージェントは他のエージェントを起動しない。** 起動はメインセッションの専権事項である。他エージェントの作業が必要になった場合、エージェントは**完了報告に要請を含めて返す**。関数が別の関数を呼ばず、呼び出し元に戻り値を返すのと同じ関係になる（§3.4 参照）
...
エージェントは他のエージェントを起動しない。起動はメインセッションの専権事項である。
...
**理由:** サブエージェントは他のサブエージェントを起動できない。「起動する」と書かれた指示は実行されず、しかも**エラーにならずに黙って飛ばされる**。書いた側は実行されたつもりでいるため、欠落に気付く機会がない。要請を戻り値に載せれば、メインセッションが起動を判断でき、実行されたかどうかも観測できる。
