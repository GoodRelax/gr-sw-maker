...
| test-designer | 受入基準とテストコードを書くエージェント。Chapter 8.1 / 9.1 / 10.1 のオーナー |
| tester | テストを実行し結果を記録するエージェント。Chapter 8.2 / 9.2 / 10.2 のオーナー |
...
| 与件（Given） vs `GIVEN` 欄 | 与件 = Chapter 2 が扱う、変えられない前提。`GIVEN` = Chapter 8〜10 のテストノードの欄で、Gherkin 由来の前提条件。**別物である。後者は必ず大文字の欄名 `GIVEN` で書く** |
