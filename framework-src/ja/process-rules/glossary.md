...
純粋性の判定は原則「クラス/関数」単位、物理分離は「モジュール/コンポーネント」単位で行う。レイヤー軸（Entity/UseCase/Adapter/Framework）はこの包含と直交し、spec-template Ch3.1・review-standards の CA に定義済み。
...
| 6 | アプリケーション (application) | — | パッケージ、外部ライブラリ |
