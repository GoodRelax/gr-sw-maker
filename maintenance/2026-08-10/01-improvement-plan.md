...
|:-:|---|---|:-:|---|
| **0** | 基準線を取る | 測定の分母 | 低 | — |
| **1** | **開発方式の整理（簡易 / 標準 / 厳格 × 条件付き 13 フラグ）** | **手順 137 → 68（簡易）/ 105（標準）** | 中 | `14` に無い |
...
| 9 | **プロセス規則 §3.2 と §3.3 を 1 つのプロセス一覧（17 件）に統合する。** 章分けの理由は表の値がすでに表している。**`コミュニケーション管理`（§3.3.2）は削る** —— 3 行の中身がすべて Fd と CLAUDE.md「重要判断の基準」と重複していた | 表 D-1 |
...
| 3 | **手順数** | `commands/full-auto-dev.md` の分岐を数える | 簡易 **68** / 標準 **105** / 厳格 **107**（全 137 手順のうち） |
