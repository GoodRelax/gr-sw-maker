...
|:-:|---|---|:-:|---|
| **0** | 基準線を取る | 測定の分母 | 低 | — |
| **1** | **開発方式の整理（規模 3 段 × Critical × 条件 13）** | **手順 137 → 69（Small）** | 中 | `14` に無い |
...
> **削減の効き方が 2 種類あることに注意する。** 段 1 は**手順の数**を減らし（Small で 110 → 69、−37%）、段 2 は**1 手順あたりのコンテキスト**を減らす（−94.2%）。**掛け算で効くが、どちらか一方では足りない。** 段 1 の削減率が段 2 より低いのは、Phase 0 の 19 手順が減らせず、ゲートも免除できないためである（`02-development-modes.md` §4.3）。
...
**移す対象の表は `03-mode-matrix.md`（表 0・A・B-1・B-2・C・D-1・D-2）。判断の根拠は `02-development-modes.md`。** 本書では位置づけと完了条件のみを記す。

| # | やること | 出どころ |
|:-:|---|---|
| 1 | **決めるために見るものを 1 つにする** —— 規模 **3 段**（Small/Standard/Large）× Critical フラグ × 条件付き 13 フラグ | §4.1 |
| 2 | **`Micro` を `Small` に統合する。** 隣り合う区分の差を数えた結果、決まった差は R3/R6 の 2 行だけだった | §4.1b |
| 3 | **「任意」を全廃する。** 5 件をすべて免除に倒す。セルの値は 必須 / 免除 / 条件付き の 3 つだけ | §0-5 |
| 4 | **表 A〜D を `プロセス規則 §3.1.1` に置く**（仕様形式 / フェーズと手順 / エージェント / プロセスと成果物） | §4.2〜4.5 |
| 5 | **`commands/full-auto-dev.md` Phase 0 に `0b3. 開発方式を決める` を足す**（現在、規模による分岐が 0 件） | §5 |
...
| 8 | **`中規模以上` と `Micro` を廃し、用語集に非採用語として登録する** | §4.5 |
| 9 | **プロセス規則 §3.2 と §3.3 を 1 つのプロセス一覧（17 件）に統合する。** 章分けの理由は表の値がすでに表している | 表 D-1 |
| 10 | **SAST と SCA を別プロセスに分ける。** SCA は全規模必須、SAST は Small で条件付き | 表 D-1 |
| 11 | **Small の必須プロセスを 8 → 4 に絞る**（トレーサビリティ管理・監査記録管理・ライセンス管理・SCA）。**§3.1.1 のルール行「defect/CR 記録・リスク管理・トレーサビリティ・変更管理は全規模で必須」を改める** | 表 D-1 |
| 12 | **`tools/spec-query/ancestors.jq` を新設する**（段 6 で実施）。`SWS` の UID から祖先の鎖を返す。**implementer が読む単位を「章」から「祖先ノード」に変える** | 表 A |

**完了条件は `02-development-modes.md` §7 の 12 件。** 主なものを再掲する。

| # | 判定 |
|:-:|---|
| 1 | `commands/full-auto-dev.md` に規模による分岐が **1 件以上**（現在 0 件） |
| 2 | `CLAUDE.md` に「スケールダウン設定」節が在る |
| 3 | **免除マトリクスに `任意` が 0 件**（現在 5 件） |
| 4 | `中規模以上` と `Micro` が `framework-src/ja` に 0 件、**用語集に非採用語として登録** |
| 5 | **`tools/check-mode-matrix.mjs`（新設）が PASS** —— 表 A〜D の規模名・エージェント名・プロセス名が §3.1.1・`agent-list.md` §1・§3.2/§3.3 の正と一致する。**`任意` を値として持つセルを落とす** |
...
| 3 | **手順数**（Small） | `commands/full-auto-dev.md` の分岐を数える | 110 → 68〜69（−37%） |
| 4 | **仕様書ロード行数**（implementer・Large） | 段 6 の表 | 664 → 137 行（−79%） |
