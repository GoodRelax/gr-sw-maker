...
**フェーズがずれるので、記号の頭文字も全部ずれる。** 下表は**旧 Phase 3（新 `Phase 4` 設計）**のぶんだけ確定している。

> **他フェーズの対応表はまだ無い**（`05-open-questions.md` H1・H2）。**作業表への統合で手順の数が動いたため、数が固まってから作る。**

| フェーズ | 変更 | 新しい記号 |
|---|---|---|
| **`Phase 0`** | **新設** | **`0a`〜（§9 で確定）** |
| `Phase 1`（旧 Phase 0） | 13 手順を 1 つに統合し、開発方式の判定とステークホルダー登録簿を新設する | `1a`〜`1h`（8） |
| `Phase 4`（旧 Phase 3） | `3d` を削除し、`3a2` `3g2` の枝番を解消する | `4a`〜`4m`（13） |
| `Phase 5`（旧 Phase 4） | `4c2` の枝番を解消し、**SAST を新設する** | `5a`〜`5i`（9） |
| `Phase 6`（旧 Phase 5） | `5c2` の枝番を解消し、**旧 `5a` `5b` を「作成 / 実行」に割る** | `6a`〜`6i`（9） |
| `Phase 7`（旧 Phase 6） | `6f2` `6f3` `6g2` の枝番を解消し、**リリース判定チェックリストを新設する** | `7a`〜`7l`（12） |
| `Phase 2` `Phase 3` `Phase 8`（旧 1・2・7） | 手順の増減なし。**頭文字だけずれる** | — |

### 6.4 旧 Phase 3（新 `Phase 4`）の対応
