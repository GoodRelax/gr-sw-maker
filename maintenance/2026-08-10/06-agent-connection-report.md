...
| **サブエージェントの TTL が 5 分・主会話が 1 時間・読むたびに寿命が戻る** | **公式ドキュメント ＋ 実機検証 2 通り**（8 分空けた再開で `read=0`／`cache_creation` の内訳が主 1h 100%・エージェント 5m 100%） | §4.4 |
...
| 22 エージェントが `Agent` / `SendMessage` を持たない | **実測**（`framework-src/ja/agents/*.md` を全数検査） | §4.5 規約 7 |
...
| **キープアライブと作り直しの分岐点が約 20 分（ping 約 5 回）** | **実機検証**。机上の「12.5 往復・約 50 分」は 1 回目の再開費用を過小評価していた | §4.4 規約 1 |
| **サブエージェントの文脈の下限が 27k〜35k** | **実測 8 エージェント** | §3.4 ／ §3.8 |
...
|:-:|---|---|---|---|---|
| 1 | **サブエージェント** | 主セッション → エージェント | エージェントは**要約を返すだけ** | 既定で使える | **主軸として使う** |
| 2 | **`SendMessage` による再開** | 主セッション → 完了済みのエージェント | 呼び出し元 → エージェント（一方向） | 既定で使える | **レビュー往復に使う** |
| 3 | エージェント間 `SendMessage` | エージェント → 別のエージェント | 双方向 | 送る側の `tools` に `SendMessage` が要る。**実機で到達を確認済み** | **当面使わない**（設計判断）|
| 4 | エージェントチーム | lead → teammate 群 | teammate 同士が直接 | **実験的・既定で無効** | **使わない** |
| 5 | 入れ子（エージェントがエージェントを呼ぶ） | エージェント → エージェント | 上位へ要約が返る | `tools` に `Agent` が要る | **使わない。層数を 1 に固定して構造で止める** |
...
**寿命が効くのは受領通知の側ではなく、指摘 → 修正 → 再確認の側である。** 修正が 5 分を超えるとエージェントのキャッシュは消える。**キープアライブと作り直しの分岐点は実測で約 20 分。** 既定はキープアライブせず作り直す。詳細は `07-agent-orchestration-rules.md` §4.4。

#### 2.5.5 指摘 10 件は 1 通にまとめる

**往復を 1 回増やすたびにエージェントは文脈を丸ごと読み直す。** 実測で `read(n)` は `create(1..n-1)` の総和と厳密に一致した（34,927 → 60,715 → 72,147）。

| エージェントの文脈 87,769 | 1 件ずつ（10 往復） | まとめて 1 通 |
|---|---:|---:|
...
**文脈 87,769 のエージェントを 4 分間隔で 2 回再開した。**
...
**これは §2.5.1 の表の読み方を縛る。** あのエージェントは文脈 29,795 でほぼ全部が基盤層だったため「再開がほぼ無料」に見えていた。**ファイルを読んだエージェントでは 1 回目に必ず会話層を作り直す。**

**サブエージェントの下限は 27k〜35k**（実測 8 エージェント）。どんなに絞ってもここより下には行かない。
...
**`subagentStatusLine` が受け取る項目:** 表示中の全サブエージェントの行を 1 つの JSON でまとめて受け取る。各要素が `id` / `name` / `type` / `status` / `description` / `label` / `startTime` / `model` / `effort` / `contextWindowSize` / `tokenCount` / `tokenSamples` / `cwd` を持つ。**`tokenCount` を `contextWindowSize` で割ればエージェントごとのコンテキスト使用率が出る**（公式が明記）。
