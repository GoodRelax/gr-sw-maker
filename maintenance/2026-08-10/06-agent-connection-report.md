...
| **サブエージェントの TTL が 5 分・主会話が 1 時間・読むたびに寿命が戻る** | **公式ドキュメント ＋ 実機検証 2 通り**（8 分空けた再開で `read=0`／`cache_creation` の内訳が主 1h 100%・体 5m 100%） |
...
| **1 回目の再開だけが高い（形の遷移）。2 回目以降は安い** | **実機検証**（64,049 → 8,391 相当） |
| **保温と作り直しの分岐点が約 20 分（維持 ping 約 5 回）** | **実機検証**。机上の「12.5 往復・約 50 分」は 1 回目の再開費用を過小評価していた |
| **サブエージェントの文脈の下限が 27k〜35k** | **実測 8 体** |
...
**`07-agent-orchestration-rules.md` §2 が持つ。本書に再掲しない。**
...
**寿命が効くのは受領通知の側ではなく、指摘 → 修正 → 再確認の側である。** 修正が 5 分を超えると体のキャッシュは消える。**保温と作り直しの分岐点は実測で約 20 分。** 既定は保温せず作り直す。詳細は `07-agent-orchestration-rules.md` §2.4。

#### 2.6.6 指摘 10 件は 1 通にまとめる
...
**「まとめて 1 通」の最悪（104,349）が「1 件ずつ」の最良（139,568）より安い。** どう転んでもまとめたほうが得である。1 件ずつの費用は返答ペースに支配され、1 件の修正が 5 分を超えるたびにその往復が 8,391 → 104,349 の 12 倍に跳ねる。**分類（争う / 直した / 保留）を付けて 1 通で送り、決着しない争点だけ 1 件ずつに落とす。** 詳細は `07-agent-orchestration-rules.md` §2.3。

#### 2.6.5 1 回目の再開は必ず高い（実測）
...
**これは §2.6.1 の表の読み方を縛る。** あの体は文脈 29,795 でほぼ全部が基盤層だったため「再開がほぼ無料」に見えていた。**ファイルを読んだ体では 1 回目に必ず会話層を作り直す。**
