...
起動するエージェントは同じ 22 体（条件付き含む）である。差は次の 14 行に出る。
...
| 14 | `02-spec-writing-rules.md` の章・節の題名が `01-spec-template.md` の見出しと一致する |
| 15 | 既知の欠陥（表 C に存在しないエージェント名を 1 件仕込む）で FAIL する |
