...
| Preconditions / Triggers / Guarantees | **採らない。** Chapter 9.1 の `GIVEN` が担う |
...
| 6.3 以降 | 候補制。下の候補表から取捨選択する。**骨格には含まれない** |
...
**`TEST_LEVEL` を持つのは `SW_SPEC_TEST` だけである。** ユースケーステストと非機能テストはレベルを持たない。
...
| 12 | **親として書いた `UID` は同じ文書の中に実在しなければならない（MUST）** | `references parent requirement which doesn't exist`。**空欄フォームだけを並べた骨格は export できない** |

> **規則 6 と 10 は、規則 7 と組で効く。** どちらも「エディタの自動整形に耐えるか」の規則である。**書いてあっても人は守れない**（§5.2 で 2 件壊れた）ので、**骨格そのものを整形の不動点にしておく。**
...
**型ごとの具体は各章の記入例にある** —— `GOAL` は Ch1.3、`USE_CASE` は Ch3.2、`FUNC_REQ` は Ch4.1、`NON_FUNC_REQ` は Ch4.2、`SW_SPEC` は Ch6.1、テストの 3 型は Ch9.1 / Ch10.1 / Ch11.1、`TEST_RESULT` は Ch9.2 にある。**同じ例を本節にも置いてはならない（MUST NOT）** —— 片方だけが更新される。**空欄のフォームは骨格（`spec-template.md`）が持つ。**
...
**検査の入力に骨格そのものを含める（MUST）。** `spec-template.md` は文法ファイルと並べて置けば単独で export が通る。**骨格が壊れていることを、それを写した仕様書が出てくるより先に知る手段がこれである。**
...
| A.2 | 依存ライブラリのライセンス情報 |
| A.3 | 本文書のバージョン履歴 |
| A.x | プロジェクト固有の補足資料。**骨格には含まれない** |

---

> 以下の「Design Rationale」と「References」は本書自体の設計根拠と参考文献である。**プロジェクトの仕様書へ写すものではない。** 本書は骨格と違ってコピーされないので、ここに理由を寄せておけば、写された仕様書が理由の重みを背負わずに済む。

## Design Rationale (本構成の設計根拠)

| 判断 | 根拠 |
|---|---|
| **骨格と規則を別の文書に分ける** | **読み手が違う。** 骨格を写すエージェントは規則の散文を毎回読み込む必要がなく、規則を引くエージェントは空欄を読み飛ばす必要がない。**総量は減らない。減るのは 1 回あたりに読ませる量である** |
| **骨格を「そのまま export が通る 1 枚」にする** | **骨格に混入した fault は、生成される全プロジェクトの仕様書に複製される。** 骨格が実行可能なら、壊れたことを写される前に機械が知る。**規則文に書くだけでは防げない**（エディタの自動整形は規則文を読まない） |
