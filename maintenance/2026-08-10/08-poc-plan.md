...
| **5** | **`02` を 10 章化して配布** | 1510 → 1445 行。19 件すべて適用。ANPS の枚数 15 → 14 | 4 | **済**（2026-08-11） |
| **6** | **`07` を整理して配布** | 604 → 571 行。保守記録 8 種を削除。`06` 参照 24 件は 20 件を数値ごと本文へ取り込み、4 件を落とした | 10・11 | **済**（2026-08-11） |
| **7** | **`test-designer` と `tester` に引用表を足す** | 2 ファイル | 9 | **済**。`context-census` が FAIL → PASS（24 体・削減率 93.9%） |
...
**いずれも PoC を止めない。** ただし **`terminology-checker`（旧 `kotodama-kun`）は走るようになった**（2026-08-12）。Out を生成した体が完了報告に用語チェック要請を含めて返し、`main-agent` が起動する。**用語検査は発火するので、上の 9 件は当てておくほど指摘が減る。**
...
| ~~`kotodama-kun`~~ | **決着した**（2026-08-12）。**エージェントのまま `terminology-checker` へ改名。** 道具化は撤回した —— 6 観点のうち機械で決まるのは非採用語との照合だけで、和製英語・同義語・品詞は意味理解を要する。**呼び出しは完了報告の用語チェック要請に統一**（`agent-orchestration-rules.md` §4.6 の規約 5・6）。モデルは `sonnet` に揃えた |
| ~~エージェント名簿の目標件数~~ | **決着した**（2026-08-12）。**22 件。** `test-engineer` を削除し（作業表で 1 度も使わない）、`terminology-checker` は名簿に残す（完了報告で呼ばれる） |
