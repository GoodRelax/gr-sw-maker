...
| # | 判断 | 決定 | 根拠 |
|:-:|---|---|---|
| 4 | **規模の段数** | **3 段: Small / Standard / Large。Micro を Small に統合** | §4.1b |
| 5 | **「任意」の扱い** | **全廃。** 5 件をすべて **免除** に倒す | 全自動開発で「任意」は走行ごとに答えが変わる。**決めていないという意味しか持たない** |
| 6 | R3 / R6 レビュー報告（Small） | **最終レビューに統合**（旧 Micro の値を採る） | §3.1.1 のルール行が「レビューは統合してよいがスキップは禁止」と既に認めている |
| 7 | Standard の仕様形式 | **ANPS-part（4 枚）** | 15 枚の管理費が Standard に見合うかが未検証。Large で先に運用して測る |
| 8 | 表 D の Large 列 | **残す**（Standard と同値でも） | 表 A〜D で区分名を揃える。**「表 D では Standard と同じ」と明記する** |
...
**§8 にあった 2 件は決着した。結果は `03-mode-matrix.md` にある。**

| 何 | 決定 |
|---|---|
| Small で 1d（モック / PoC）を行うか | **全規模で実施する。** UI が変わることと、最初にイメージを合わせることは別問題である |
| Small で起動しない体 | risk-manager / change-manager / process-improver / decree-writer / progress-monitor / user-manual-writer / runbook-writer / incident-reporter / 実機テスト 3 体。**Small は 10 体** |

**あわせて `03-mode-matrix.md` で次を決めた。**

| # | 決定 | 効果 |
|:-:|---|---|
| 1 | **プロセス規則 §3.2 と §3.3 の表を統合する** | 章が分かれていた理由（全プロジェクト共通か中規模以上か）は表の値がすでに表している。プロセス 17 件の 1 表になる |
| 2 | **SAST と SCA を分ける** | 別物である。SCA はコマンド 1 つ（全規模必須）、SAST は CI 設定が要る（Small は条件付き） |
| 3 | **Small の必須プロセスを 8 → 4 に絞る** | 残すのはトレーサビリティ管理・監査記録管理・ライセンス管理・SCA。**現行 §3.1.1 のルール行を改める** |
| 4 | **implementer が読む単位を「章」から「祖先ノード」に改める** | `SWS` の祖先は 3 ノードであって 3 章ではない。**`ancestors.jq` を新設する**（段 6） |
