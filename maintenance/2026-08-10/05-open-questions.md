...
| # | 論点 | なぜ要るか |
|:-:|---|---|
| C6 | **`ENABLE_PROMPT_CACHING_1H=1` はサブエージェントにも効くか** | **前提は実測で確定した**（下記）。**環境変数を入れた場合が未検証。** 効けば §16.11 の保温という選択肢自体が不要になる |
...
| **C12** | **厳格のレビュー分野の割り方** | **推奨案は「1 観点 = 1 体、R をまたがない」**（下記）。**利用者のレビュー待ち** |
| **C13** | **`kotodama-kun.mjs` が見る用語集の範囲** | `check-terms.mjs` はフレームワーク自身の用語集を見る。**成果物に効かせるにはプロジェクト用語集も見る必要がある。** 実装時に決める |

**C12 の推奨案 —— 1 観点 = 1 体。R をまたがない。**

| 手順 | 観点 | 簡易・標準 | 厳格 |
|---|---|---|---|
| 要求レビュー | R1（6 項目） | 1 体 | **1 体**（割る先が無い） |
| 設計レビュー | R2/R4/R5/R7 | 1 体 | **4 体** |
| 実装レビュー | R2/R3/R4/R5/R7 | 1 体 | **5 体** |
| 最終レビュー | R1〜R7 | 1 体 | **7 体** |

| # | 採る理由 |
|:-:|---|
| 1 | **追跡性が切れない。** 独自の分野名を作ると `review-standards.md` との対応表が要り、二重管理になる |
| 2 | **プロンプトが 1 行で済む**（「R2 だけ見よ」）。分野の定義を書き下ろさなくてよい |
| 3 | **最終レビューの 7 観点が 4 分野に収まらない問題が消える。** 同時実行の上限 20 に対して余裕がある |

**項目数の偏り（R2 が 20、R4 が 3）は許容する。** 割る目的は負荷分散ではなく**文脈の分離**であり、早く終わる体は損ではない。

> **費用を明示しておく。** 体には下限（27k〜35k）があるため、最終レビューを 7 体で回すと**下限だけで 7 倍**になる。簡易・標準の約 38k 相当に対し、**厳格は約 260k 相当。これが厳格の値段である。**

> **C8（体の文脈量の実測）は解決した。** 実測でサブエージェント 1 体の文脈は 30k〜88k、**下限は 27k〜35k**（8 体）であった。**§16.11 の分岐点も実測で約 20 分に確定した**（机上の「12.5 往復・約 50 分」は 1 回目の再開費用を過小評価していた）。この項目は不要になった。
