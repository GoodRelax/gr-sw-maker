...
> **本書は 2026-08-09 に分割された。以降の正本は次の 2 枚である。**
>
> | 分割後 | 中身 | 適用先 |
> |---|---|---|
> | `03-spec-template-skeleton.md` | **骨格。** そのまま `strictdoc export` が通る 1 枚 | `framework-src/{lang}/process-rules/spec-template.md` |
> | `04-spec-writing-rules.md` | **規則。** 本書から骨格を抜いた残り全部 | `framework-src/{lang}/process-rules/spec-writing-rules.md`（新設） |
>
> **本書は、記法を実測に合わせて直し終えた時点の記録として残す。以降は直さない。** 分割の経緯と判断は `05-step2-split.md` にある。
...
| 1.5 | Scope | 範囲 | 本プロジェクトでやること (In-scope) とやらないこと (Out-of-scope)。**機能の範囲を書く。物理的な範囲（どの機器までが対象か）は Chapter 2 に書く** |
...
| Section | English | 日本語 | 記述内容 |
|---|---|---|---|
| 2.1 | Overview Diagram | 概要図 | 機器と経路の全体図。Mermaid |
| 2.2 | Devices | 機器 | 機器の一覧。対象ソフトが載る機器を明示する |
| 2.3 | Routes | 経路 | 機器の間、および人と機器の間の経路。何が流れるかを書く |
| 2.4 | Exclusions | 持たないもの | 構成に含まれないものを明示する |
...
**Alistair Cockburn, *Writing Effective Use Cases*（Addison-Wesley, 2001）。** 本テンプレートは brief を土台とし、casual と fully dressed は採らない。
...
| 5.2 | Components | コンポーネント | 常に | 部品と責務の分割。**コンポーネント図**（5.1 の凡例で色分け）。**各コンポーネントがどの機器（Chapter 2.2）に載るかを書く。** AI/LLM 連携がある場合はプロンプトテンプレートの配置（`src/prompts/` 等）・入出力スキーマ・テスト方針・ハルシネーション対策も定義する |
...
| 5.5 | Behavior | 振る舞い | 常に。**ただし図は種類ごとに条件が異なる** | 処理フロー・相互作用。状態遷移図（**持続する状態を持つ場合**）、シーケンス図（**複数コンポーネントの相互作用がある場合**）、アクティビティ図（**分岐・並行が多い場合**）。**機器をまたぐ相互作用は、Chapter 2.3 の経路を名前で明記する** |
...
| **目標レベルを `sea` に限る** | `kite` は複数の `sea` をまとめ、`fish` は他の `sea` から使われる。**どちらも「どの UC と繋がるか」を書く場所を要するが、本テンプレートは入れ子を作らない**（Cockburn の Linking Use Cases を採らない）。関係を書けないまま階層だけ作ると、部分機能が独立した利用者目標に見える |
...
10. **StrictDoc** — 本テンプレートが配る文法（`spec.sgra`）と Markdown 記法の実装。**実測環境: strictdoc 0.27.1 / Python 3.13.3 / jq 1.8.1 / Windows 11**
