...
| # | 対象 | 内容 | 判定 |
|:-:|---|---|---|
| 5-1 | `full-auto-dev-document-rules.md:1186` | traceability の実装列「ファイルパスまたはモジュール」 | **変更しない。** 新定義のもとで文がそのまま成立する。ファイルパスはユニットを指す書き方であり、モジュールはサブフォルダを指す。**要求 1 件が複数ユニットにまたがる場合に粗い粒度で書けることは traceability にとって有用**であり、両方を許す現状が正しい |
...
| ステップ | 状態 | 備考 |
|---|---|---|
| 1 | **適用済み** | glossary §5 を 3 軸に再構成。ユニットを定義、番号を廃止、公開面（§5.3）を新設 |
| 2 | **適用済み** | review-standards 8 箇所 + R2.19 新設 + チェックリストに独立見出し |
| 3 | **適用済み** | **3-3 はステップ 2 に前倒しした。** R2.19 が Ch3.3 の公開面宣言を参照するため、同一コミットに含めないと存在しない宣言先を指す状態になる |
| 4 | **適用済み** | test-engineer の describe、process-rules の「モジュール単体テスト」 |
| 5 | **判定済み** | 変更なし（上表） |

**残存数の実測:** 着手前 44 → ステップ 1 後 48 → 全適用後 **36**。§5 の予測値と一致した。
...
| # | 内容 |
|:-:|---|
| 1 | 本改訂を OKF 移行の前に行うか後に行うか。**glossary は Common Block を持たないため OKF 移行の対象外であり、独立に進められる**（設計案 §7-4） |
| 2 | 既存の `02-structural-unit-taxonomy.md` を本リストの決定で改訂するか、設計案として据え置くか。**過去記録の改竄を避けるため、据え置きを推奨する** |
| 3 | **`spec-template.md` Ch3.3 の公開面宣言に、記法の例を添えるか。** 現状は「宣言する」とだけ定め、書き方は言語機構と用語集 §5.3 に委ねている。**本走行で architect が何を書くかを観測してから判断する** |
