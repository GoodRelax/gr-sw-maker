# gr-sw-maker 初回トライアル 総括レポート v2

**対象:** `gr-sw-maker-trial`（unit-converter-cli）。2026-07-26 開始、2026-07-30 完了、GATE-DELIVERY PASS。

**v1 からの変更:** 外部の 2 資料を読み、対策の質を上げた。**問題を 22 件から 24 件に、真因の分析に外部知見の適用可否を追加した。**

| 資料 | 反映先 |
|---|---|
| Claude Code システムプロンプト削減の分析（ai-revolution.co.jp） | **§3.1**、新規 P-23、真因 B の対策 B-6 / B-7 |
| Open Knowledge Format (OKF) v0.2 仕様（GoogleCloudPlatform/knowledge-catalog） | **§3.2**、新規 P-24、真因 E / F / A の対策 |

**方針:** 数値はすべて成果物からの実測である。**外部資料の主張をそのまま採らず、gr-sw-maker の実測値と突き合わせて採否を判断した。結果として大半は不採用である。**
...
| P-15 | 全期間 | 伝聞由来の断定が事後の安全網でしか検出されない | **OKF の `sources` を Common Block に採用**（F-5） | **未着手** | 真因 F。**v2 で対策を差し替え** |
...
| **P-24** | 全期間 | **人間の承認と機械の生成を区別する形式がない** | **OKF の actor 規約を採用**（E-5） | **未着手** | 真因 E。**v2 で新規**。OKF 検討により発見 |

**適用済み 2 / 部分適用 2 / 判断待ち・未着手 20。**
...
## 3.2 Open Knowledge Format (OKF) v0.2

> **【2026-07-31 撤回】本節の結論「13 要素中 11 を不採用」は誤りである。** 却下の根拠 2 点はいずれも仕様の誤読であった。適合規則は**消費者**への制約であり生産者を縛らない。かつ独自キーは *"Producers MAY include any additional keys"* と明示的に許可されている。**訂正と採用の決定は `01-trial-report_v3.md` §3.2 にある。本節は誤りの記録として残す。**

### 仕様の要点

| 項目 | 内容 |
|---|---|
| 形態 | **YAML frontmatter 付き Markdown のディレクトリ。** 中央レジストリなし、必須ツールなし。`cat` と `git clone` だけで流通する |
| 必須 | `type` のみ（自由定義） |
| 出所 | `sources: [{ id, resource, author, usage_count, last_modified }]` + `usage_window`。**信頼スコアは保存せず、客観的シグナルのみ記録する** |
| 信頼 | `generated: { by, at }` / `verified: [{ by, at }]`。**信頼階級を frontmatter から自動導出**（unverified / machine-confirmed / human-reviewed） |
| アクター規約 | `<producer>/<version>`（エージェント）／ `human:<id>`（人間）／ `process:<id>`（自動処理） |
| ライフサイクル | `status`（draft / stable / deprecated）／ `stale_after` |
| 索引 | ディレクトリごとの `index.md` による progressive disclosure |
| 適合規則 | 消費者は**任意ファミリの欠落を理由に拒否してはならない（MUST NOT）**／壊れたリンクを許容する／未知の type とキーを許容する |

### gr-sw-maker との対応と採否

| OKF の要素 | gr-sw-maker の現状 | 採否 | 理由 |
|---|---|:---:|---|
| `type`（必須・自由定義） | `file_type`（**閉じた 37 種**） | **不採用** | 自由定義にすると Form Block の機械検査が成立しない |
| **`sources`（出所 + 信頼シグナル）** | **存在しない** | **採用** | **真因 F に直接効く。** §4.6 参照 |
| **actor 規約**（`human:` / `process:` / agent） | `by="orchestrator"` 等。**人間と機械を区別しない** | **採用** | **真因 E に効く。** §4.5 参照 |
| `generated: { by, at }` | Footer の `change_log` entry | **不採用** | 既存で足りる。二重管理になる |
| `verified` + 信頼階級 | レビュー結果は `review` file_type に分離 | **部分採用** | **階級の概念のみ借りる。** フィールドは新設しない（§4.5） |
| `status` | `document_status`（4 値） | **不採用** | 既存で足りる |
| `stale_after` | 存在しない | **保留** | **真因 C は依存ベースであり、時間ベースでは解けない。** 誤った安心を与える |
| `index.md` progressive disclosure | CLAUDE.md に規則の一覧がある | **不採用** | **二重管理になる。** SSOT 規律に反する |
| `log.md` | Footer の `change_log` | **不採用** | 既存で足りる |
| 壊れたリンクの許容 | `check-links` が FAIL にする | **不採用** | ゲート思想と衝突する |
| **任意ファミリ欠落での拒否禁止** | **In の必須要素検査（MUST）** | **不採用** | **正面から衝突する。** 契約は入口で検査するのが本フレームワークの設計原則 7 |
| `Attested Computation` | 該当する成果物がない | **保留** | 性能測定の再現性に将来使える可能性がある |
| `okf_version` の宣言 | — | **不採用** | **部分採用であり適合しないため、OKF を名乗ってはならない** |

### 採用の性格を明確にする

> **gr-sw-maker は OKF に準拠しない。適合規則（任意ファミリの欠落で拒否してはならない）が本フレームワークの入口検査と正面から衝突するためである。**
>
> **採用するのは `sources` と actor 規約の 2 つであり、語彙と設計を借りるにとどめる。** 文書に `okf_version` を宣言してはならない（MUST NOT）。準拠していないものを準拠と書けば、それ自体が本試行で 3 回起きた「伝聞を検証せず断定へ変えた」型の誤りになる。
...
| **5** | **E-5 actor 規約 + F-5 `sources` の採用**（OKF） | E, F | **2 つの真因に同時に効く。仕様が既にあり自前発明を避けられる** |
| **6** | C-3 改訂時の追随確認（図 4 種の列挙） | C | **実証済みの真因。** 対象が具体化できている |
| **7** | F-4 非採用語の機械検査 | F | **最多かつ最安** |
| **8** | B-5 Ch3 に適用場面列 | B | 過剰設計の直接原因 |
| **9** | **B-6 規則追加への最小構成の併記 + B-7 既定値と MUST に理由を併記**（H-1 と統合） | B, H | **R2.18 の自己適用。** 記法を 1 つにまとめる |
| **10** | G-4 提案の滞留の可視化 + **G-6 decree-writer の適用範囲の明記** | G | 軽い。忘却と誤読を防ぐ |
| **11** | P-10 `.gitignore`、P-20 コマンド名 | H | 独立。いつでもよい |
| **12** | **E-2 orchestrator の改名**（517 箇所） | E | **単独の change-request。** 次走行の前後を別途判断 |
| **13** | 2 回目のトライアル | — | R2.18 の初検証を含む |
