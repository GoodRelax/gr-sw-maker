...
| # | フェーズ | 問題 | 対策 | 対策状況 | 備考 |
|:--:|---|---|---|:---:|---|
| P-01 | 全期間 | `session-state.json` が一度も生成されず、コスト・コンテキストが計測不能 | 原因調査（D-1）+ 経路断絶の検知機構（D-2） | **適用済み** | FF-06。真因 D。D-1: statusLine はデスクトップで発火しないと特定。D-2: `tools/otel-sink.mjs` の鮮度マーカーと §3.2.7 の検査。**受け口の起動は走行手順に残る** |
| P-02 | 全期間 | 改訂が従属記述に追随しない。design だけで新規 7 件 | **従属しやすい 4 種類を列挙**し改訂時の追随確認を義務づける（C-3） | **適用済み** | FF-04。真因 C。文書管理規則 §6.1 + architect 手順 14。C-1（change_log への記録）を併用 |
| P-03 | setup | CLAUDE.md テンプレートがサーバー / Web 前提で、CLI と 6 箇所衝突 | 既定値に「代替を採る場合の記録先」を併記（H-1） | **部分適用** | 真因 H。**規約は §5.6 として確立**（B-7 と統合）。テンプレート本体は 5 箇所適用済みで、ブランチ戦略・SAST ツール名・可観測性 4 行が未適用 |
...
| P-06 | design | spec-template の Ch3 に「適用場面」列がない（Ch4 にはある） | Ch3 に適用場面列を追加（B-5） | **適用済み** | C-2。真因 B。図ごとの条件を明記し、省略時は Ch3.6 に理由を記録させる |
...
| P-09 | 全期間 | 値域が現実の状態を表現できない。**4 件** | **値域設計の原則**を §9 に新設（A-2）+ 既知 5 件の修正 | **適用済み** | FF-05 / FF-07 ほか。真因 A。§9.0.1 新設。`project_status` / verdict 拡張 / cost 欠測許容 / `approved_chapters` |
| P-10 | setup | `.gitignore` が規則の義務づけた保存先 `old/` を除外している | `old/` を除外対象から外す | **適用済み** | FF-01。真因 H。**実バグ**。gitignore-user.template から除去し、理由を併記 |
| P-11 | setup | 「セキュリティ脆弱性 Critical: 0」がどの重大度尺度を指すか未定義 | SAST / SCA を指すと CLAUDE.md に明記 | **適用済み** | FF-02。真因 E。**尺度はスキャナ自身の格付けと定め**、ツールと尺度の記録を setup で義務づけ |
| P-12 | planning | 「解決済み」分類の内側に未点検の論点が残る | 分類の粒度自体を検証する観点を追加 | **未着手** | FF-03。真因 C |
| P-13 | planning | `spec-foundation` に章単位の承認状態を表すフィールドがない | 承認状態フィールドの新設 | **適用済み** | FF-05。真因 A。`spec-foundation:approved_chapters` を新設 |
| P-14 | 全期間 | 非採用語の混入が **4 回**。delivery では宣言自体が自己矛盾 | **機械検査**（`tools/check-terms.mjs`）（F-4） | **適用済み** | 真因 F。CI に登録。**framework-src 自身の違反 19 件も是正した** |
| P-15 | 全期間 | 伝聞由来の断定が事後の安全網でしか検出されない | **OKF の `sources` を frontmatter に採用**（F-5） | **適用済み** | 真因 F。文書管理規則 §5.2。適用範囲は数値と他者の判断の引用に限定 |
| P-16 | design | フェーズ完了条件（`test-plan`）を検証する者が未定義のまま次フェーズへ | ゲート判定要請にフェーズ完了条件の充足状況を添付 | **適用済み** | 真因 E。technical-authority 手順 3b に新設。**添付が無ければ差し戻す**ため、要請側の義務が生じる |
| P-17 | 全期間 | ふりかえりの提案が一度も適用されない | 滞留の可視化（G-3）+ 次プロジェクト開始前の適用（G-2） | **適用済み** | 真因 G。final-report に未適用件数と最古提案日を必須化。decree-writer の起動条件に「走行中でない」「構造的と判定済み」を追加 |
| P-18 | 全期間 | メインセッションが名簿にも §11 にも存在しない | §11 の owner 値域に `main-session` を追加 | **適用済み** | B-2。真因 E。**サブエージェントが復元できない情報を持つ成果物に限る**条件付き。名簿には追加しない |
| P-19 | 全期間 | orchestrator の名前が実態と乖離。progress-monitor と同じ名乗り | 改名（`project-manager`）と名乗りの分離 | **適用済み** | B-3 / B-4。真因 E。**E-1 適用済み**（窓口記述の削除・名乗りの分離）。**改名 E-2 も適用済み**（`967cfde` / `d508c5b`。glossary と agent-list に退役理由を記載） |
| P-20 | 全期間 | コマンド名が世の中の一般名と衝突しうる | **`session-handoff` の新設**（名称を再検討して変更） | **適用済み** | B-1。真因 H。**衝突リスクはコマンド名前空間のみで、file_type 名には及ばない**（`/retrospective` → `retrospective-report` の先例）。接頭辞（B-6）は未決のまま |
| P-21 | setup | `budget_usd`（予算絶対額）の入れ物が CLAUDE.md にない | 行を新設する | **判断待ち** | 真因 D |
| P-22 | 全期間 | decree-writer の適用先が配布物であり、原本 `framework-src/` に届かない | **還元経路を作らず、責務の限界を定義に明記**（G-6） | **適用済み** | 真因 G。適用先をプロジェクト配下に限定（framework-src は MUST NOT）。setup.js による上書きも明記 |
| **P-23** | 全期間 | **フレームワーク自身が R2.18 の対象外。11,264 行が最小構成と比較されたことがない** | **規則追加の提案に最小構成の併記を義務づける**（B-6） | **適用済み** | 真因 B。framework-development.md §5.5。**§5.6 が自身に §5.5 を適用して見せている** |
| **P-24** | 全期間 | **人間の承認と機械の生成を区別する形式がない** | **OKF の actor 規約を採用**（E-5） | **適用済み** | 真因 E。§5.1。承認 4 フィールドに配線（tech-decision:approved_by を新設） |
...
**適用済み 20 / 部分適用 1 / 判断待ち 3 / 未着手 3。**

---

# 2. 問題の依存関係

27 の問題は独立ではない。**8 つの真因に収束し、真因どうしにも依存がある。**
