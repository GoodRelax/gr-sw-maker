...
| `Phase 1` 初期設定 | Phase 0 | 18 | 9 | 13 手順を 1 つに統合。開発方式の判定・ステークホルダー登録簿・**CLAUDE.md の記入**を新設 |
| `Phase 2` 企画 | Phase 1 | 9 | 11 | **目的・概要・UC を書く手順を新設**（鎖の根）。 `1i` を 2 ゲートに分割（`05-open-questions.md` A1 の決着） |
...
| `Phase 6` テスト | Phase 5 | 7 | 12 | 旧 `5a` `5b` を「作成 / 実行」に分割。**UC テストのケースと実行、非機能テストのケースを新設** |
| `Phase 7` 納品 | Phase 6 | 11 | 12 | リリース判定チェックリストを新設 |
| `Phase 8` 運用・保守 | Phase 7 | 6 | 6 | 増減なし。頭文字だけずれる |
| 共通手順 `F` | 同左 | 6 | 11 | `Fa` を削除（道具化）。defect 票・版上げ・**指摘への回答と再判定・即時エスカレーション・ゲート FAIL の差し戻し**を新設 |
| **合計** | | **86** | **96** | |
...
| `1e` | **新設** | 開発方式（簡易 / 標準 / 厳格）を決める |
| `1f` | **新設** | ステークホルダー登録簿を作る |
| `1g` | **`0c`〜`0n2`（13）** | 条件付き 13 プロセスの要否を一括で評価する（統合） |
| `1h` | `0o` | 評価結果を報告し確認を求める |
| `1i` | `0p` | pipeline-state を初期化する |
...
| `2d` | `1e` | モック / サンプル / PoC を作る |
| `2f` | `1f` | 仕様書の要求の章を作り、要求に ID を付ける |
| `2g` | `1g` | 以降の章のスケルトンを置く |
| `2h` | `1h` | 仕様書の概要を報告し承認を求める |
| `2i` | `1i` | 要求の品質レビュー（R1） |
| `2j` | **`1i` 前半** | GATE-INTERVIEW を判定する（分割） |
| `2k` | **`1i` 後半** | GATE-PLANNING を判定する（分割） |

> **2 ゲートは同じ遷移（`planning → dependency-selection`）に属する。** 分割してもフェーズ末尾に 2 つ並び、中間には置けない。**順序は GATE-INTERVIEW が先である。** GATE-PLANNING の合格条件が R1 PASS を要求し、それを出すのが `2i` だからである（プロセス規則 §9.4.1）。

**`Phase 3`（旧 Phase 2 外部依存選定）7 → 7:**

| 新 | 旧 | 手順 |
|---|---|---|
| `3a`〜`3g` | `2a`〜`2h` | 並びのまま 1 対 1 で写る |
...
| `6h` | `5c` | 性能テストを実行し結果を記録する |
| `6i` | `5c2` | 実機テストを実施し、分類・原因分析を行う |
| `6j` | `5d` | テスト消化曲線と defect curve を更新する |
| `6k` | `5e` | テストコードのレビュー（R6） |
| `6l` | `5f` | GATE-TEST を判定する |
...
**ただし出自は消さない。** `commands/full-auto-dev.md` に本体がまだ無い行を区別する必要があるためである。**記号の下に `<br />` で並べて残す**（例: `` `1e` ``<br />`**新設**`）。

`00-mode-matrix.md` §11 の検査 8 は、**手順セルに `新設` / `統合` / `分割` を含む行を実在検査の対象外とし、件数を出力する**と読み替える。現在 14 件（新設 7・統合 1・分割 6 —— `Phase 0` の 5 手順を除く）。**新設の内訳は `1e` `1f` `4f` `5g` `7b` `Fg` `Fh`。**
...
| # | プロセス | 何が無いか |
|:-:|---|---|
| 2 | 監査記録管理 | 旧 `2f`（新 `3e`）が外部依存の選定結果を記録するだけ。他フェーズの decision を起こす手順が無い |
| 13 | リリース管理 | 旧 `6b`〜`6h`（新 `7c`〜`7f`）はデプロイの手順。`release-checklist` を作り判定する手順が無い |
...
**上の 3 件は新設で埋まった。** リリース管理は `7b`、API バージョニングは `4f`、SAST は `5g` が持つ。**監査記録管理は `1e` `3e` `4b` の `出力` に `decision` が現れることで満たす。**
