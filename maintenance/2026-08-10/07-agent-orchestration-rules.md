...
| §5 注記 | 適用の限界。差し替えるときに測ること。旧番号の読み替え | — |

> **旧版は §1 考え方 / §2 手段 / §3 注記 の 3 章であった。** 節番号の読み替えと、本版で変えたことは §5 が持つ。

---

## 1. 背景

**全自動開発は 1 エージェントでは回らない。** 20 を超えるエージェントが同じ成果物を順に触り、**書いたエージェントと見るエージェントは必ず別である。** したがって受け渡しの費用と正確さが、そのまま開発の費用と品質になる。
...
| 日本語 | 英語 | 定義 |
|---|---|---|
| **エージェント** | agent | 独立した文脈を持つ実行単位。**会話履歴・呼んだスキル・読んだファイルを引き継がない**（`06` §2.3） |
...
| **背景起動 / 前景起動** | background start / foreground start | 起動のしかたの別。**再開は必ず背景実行になる**ため、前景で起動するとキャッシュが繋がらない（`06` §2.5.1） |
...
| **文脈** | context | そのエージェントが持つ全文（基盤層＋会話層）。**往復のたびに全部を送り直す**（`06` §2.5.5） |
| **基盤層** | base layer | 役割の定義・道具の一覧・プロジェクトの決まりごと。**起動のときに必ず載る** |
| **会話層** | conversation layer | それまでの依頼・返答・読んだファイルの中身 |
| **下限** | floor | 基盤層のぶんの費用。**どれだけ指示文を絞ってもここより下に行かない**（実測 27k〜35k、`06` §2.5.6） |
| **キャッシュ** | cache | 送り直した文脈のうち、前回と同じ先頭部分を安く読ませる仕組み |
| **キャッシュの寿命** | cache TTL | キャッシュが有効な時間。**5 分。読むたびに戻る**（`06` §2.5.2）。**エージェント自身の寿命とは別物である** |
...
| **保温** | 何を保つのかが語に現れない。**キープアライブに置き換えた**（本書の旧版の語） |
...
|:-:|---|---|
| 1 | **文脈が独立している。** 会話履歴・呼んだスキル・読んだファイルを見ない（`06` §2.3） | **依頼文が、そのエージェントの知る世界のほぼ全部になる。** 渡さなかったものは存在しない |
| 2 | **状態を持たない。** 往復のたびにそれまでの全文を送り直している（`06` §2.5.5） | **往復の回数がそのまま費用である**（§3.2） |
| 3 | **親にしか返せない。** エージェントから利用者へは何も届かない（`06` §2.3） | 利用者との往復はすべて `main-agent` を通る。**設計判断ではなく構造上の制約である** |
| 4 | **起動するだけで下限の費用が出る**（`06` §2.5.6） | 軽い作業をエージェントでやると、下限を毎回払う（§3.4・§3.8） |
| 5 | **キャッシュの寿命が短く、放っておくと冷える**（`06` §2.5.2） | ウォーム待機を保つか作り直すかを、あらかじめ決めておく必要がある（§3.1・§4.4） |
| 6 | **入れ子には上限があり**（`06` §1.3・§2.3）、**達しても失敗しない。起動の権限が黙って取り上げられる** | **気づけない。構造で縛るしかない**（§4.5 規約 5） |
...
| **ウォーム待機から再開** | 前回と同じ先頭部分をキャッシュから読む | **安い**（読みは書きの約 1/10、`06` §4） |
| **コールド待機から再開** | 全文を書き直す | **高い**（書きは通常の 1.25 倍、同上） |
...
> **ただし 1 回目の再開だけは、ウォーム待機でも会話層が書き直される**（`06` §2.5.6 の実測）。**2 回目以降は当たる。** だから **1 往復で終わる作業に再開を当てても得しない**（§4.1 規約 4）。

**数値は本書が持たない。** 下限が何トークンか、寿命が何分か、再開がいくらかは `06-agent-connection-report.md` が持つ。**本書は「だからこうする」だけを持つ。**
...
| 決めないこと | どこが持つか |
|---|---|
| どの手順を誰が担当し、どの経路で受け渡すか | `00-mode-matrix.md` §4・§5 の作業表 |
| レビューの観点（R1〜R7）と、どの観点がどの章を見るか | `review-standards.md` |
| 実測値と出典 | `06-agent-connection-report.md` |
| 未決の論点 | `05-open-questions.md` C 群 |

> **経路の規則の正本は本書である（MUST）。** 作業表の `関連` 列が本書と食い違ったら、作業表の側を直す。
...
**本章の図は、エージェントの名前ではなく役で書く。** 同じ形が成果物の種類を問わず成り立つからである。**誰がその役を務めるかは作業表（`00-mode-matrix.md` §4）が持つ。**
...
| # | 規約 | なぜ |
|:-:|---|---|
| 1 | **再開する見込みのあるエージェントは、最初から背景で起動する（SHOULD）。** §3.1 の 2 条件を満たす作業が該当する | 前景で起動すると、再開時にツール一覧が変わってキャッシュが全部無効になる（`06` §2.5.1） |
| 2 | **完了時に返る識別子を記録する** | 残さなければ再開できない。**そのとき捨てた識別子は後から作れない** |
| 3 | **再開の宛先は名前ではなく識別子とする（SHOULD）** | 名前は後から起動した別のエージェントに奪われうる。**別人に話しかけても気づけない** |
| 4 | **1 往復で終わる作業は再開を前提にしない。3 往復以上する見込みなら償却される** | 1 回目の再開は会話層を書き直すため、ウォーム待機でも安くならない（`06` §2.5.6） |
...
| 失うもの | どう困るか |
|---|---|
| **観測** | `main` を通らない往復は計測に出ない。**writer の内側で 10 往復してもコストログには 1 件に見える**（`06` §3.2 のとおり、モデルへ確実に届く計測値は Agent の返り値だけである） |
| **計数** | ゲート FAIL が何回目かを数えられない。エスカレーションの規則（`00-mode-matrix.md` 表 E-2）が発火しない |
...
| # | 規約 | なぜ |
|:-:|---|---|
| 1 | **既定はまとめて 1 通（SHOULD）** | 往復のたびに文脈を丸ごと送り直す（§3.2）。**まとめた最悪が 1 件ずつの最良より安い**（`06` §2.5.5） |
...
**既定はキープアライブしない。** 分岐点は実測で約 20 分（`06` §1.3）であり、**修正がそれより長引く見込みなら、コールド待機から作り直したほうが安い。**

| # | 規約 | なぜ |
|:-:|---|---|
| 1 | **修正が短時間で終わると分かっている場合にかぎり、キープアライブしてよい。** 長引く見込みならコールド待機のまま作り直す | 分岐点は実測で約 20 分（`06` §1.3）。**それを超えると、キープアライブの積み上げが再開の書き直しより高くつく** |
...
> **`ENABLE_PROMPT_CACHING_1H=1` がサブエージェントにも効くなら、寿命が 5 分から 1 時間になり、本節の選択そのものが要らなくなる。** 効くかは未確認である（`05-open-questions.md` C6）。
...
| **トークンとコスト** | 要らない | **Agent の返り値を `main` が受け取り、フェーズ境界でまとめて渡す** | モデルへ確実に届く唯一の計測値である（`06` §3.2）。**フックでは取れない**（`06` §3.1、公式で確認） |
...
| 2 | **手順の開始・終了ごとに `project-manager` を起動してはならない（MUST NOT）。フェーズ境界にまとめる** | 起動には下限（27k〜35k、`06` §2.5.6）がある。**「小さい報告を頻繁に」はエージェントでは最も高い形である** —— 手順 60 件なら 120 起動で概算 3.6M、フェーズ境界なら 9 起動で 270k |
| 3 | **各エージェントに `project-manager` へ直接報告させてはならない（MUST NOT）** | 並列実装で同じファイルへ複数のエージェントが書くと**競合する。** 調停を持つのはエージェントチームだけである（`06` §2.3）。本務に副業を足すと依頼文が伸び、下限も上がる |
| 4 | **開始の報告をエージェントにさせない（MUST NOT）** | 起動した事実は `main` が既に知っている。**知っていることを下から報告させるのは往復の無駄である** |
| 5 | **機械の記録が空のとき「順調」と読んではならない（MUST NOT）** | フックが届いていなければ黙って 0 件になる。**「0 件」と「動いていない」を区別する**（`00-mode-matrix.md` の `0d` と同じ穴） |

> **未確認が 1 つある。** フックが `tool_name` / `tool_input` を受け取ることと `SubagentStop` の発火は公式に定義があるが、**本環境でフックが Agent の返り値まで受け取れるかは確かめていない。** 上の設計は「受け取れない」前提で割ってあるので（`06` §3.1）、**受け取れたときは楽になるだけで、経路は変わらない。**

### 4.6 用語の検査は道具で回す —— `kotodama-kun.mjs`

**用語の番人はエージェントをやめ、道具にする。名前は残す。**

| # | 決定 | なぜ |
|:-:|---|---|
| 1 | **`agents/kotodama-kun.md` を廃止する。** エージェントとしては存在させない | 用語の照合は用語集の非採用欄と突き合わせるだけで、**答えが一意に決まる**（§3.8） |
| 2 | **`tools/kotodama-kun.mjs` を新設する。** 成果物の用語・命名を検査する | 道具なら起動の下限を払わない。**同じ検査を何度走らせても費用が増えない** |
| 3 | **`Write` / `Edit` の前に走るフックとして配線する** | 書いたエージェントにその場で差し戻るため、**`main-agent` には何も届かない**（§3.5） |
| 4 | **`maintenance-tools/check-terms.mjs` は残す。照合の実装は共有し、二重に持たない** | そちらは**フレームワーク自身**を CI で守る検査であり、見る対象が違う。**実装を 2 つ持つと、片方だけ直る** |
...
| 2 | **フックの差し戻しは、書いたエージェントが自分で直す。** 上位へ上げない | 直せる者の手元で閉じる。**上げると往復が増え、直す者は変わらない** |
| 3 | **新語の採否は成果物ごとに判断しない（MUST NOT）。** 用語集の育成として、まとめて行う | 成果物ごとに判断すると、**同じ語について別の答えが並ぶ** |
...
| # | 変更 | 理由 |
|:-:|---|---|
| 1 | **章立てを 背景 / 目的 / 対応方針 / 具体的運用 / 注記 の 5 章にした** | 旧版は考え方から始まり、**なぜ規則が要るのか（§1）と、何を決める文書なのか（§2）を持っていなかった** |
| 2 | **§3.1 を「レビューは」で始めるのをやめ、「何度も連携する作業は」に上げた** | 用途の名前で書くと、同じ型の他の作業（ゲート判定・実機テスト・書き直し）に当てられない。**判定は 2 条件で行う** |
| 3 | **§4.3 に「分類は被レビュー側が行う」を明記した**（規約 3） | 旧版の図は `main` が三分類していた。**指摘の当否は書いたエージェントしか判断できず、§3.5 にも反する** |
| 4 | **連携図を役（main / writer / reviewer / judge）で書き直した** | エージェントの名前で書くと、同じ形が他の成果物に当てられない。**誰が務めるかは作業表が持つ** |
| 5 | **すべての系列を依頼（実線）から始める形に直した**（§4.7 規約 6） | 旧版の §2.3 は指摘（破線）から始まっており、**誰が起こした往復か読めなかった** |
| 6 | **§4.5 に利用者を挟む往復の図を足した** | 「利用者と話せるのは `main-agent` だけ」は本書で最も引かれる規約（作業表の 6 か所）だが、**形を示していなかった** |
| 7 | **§4.3 に「`main` が回送するのは場所・件数・重大度だけ」を明記した**（規約 4） | **中継の費用はここでしか出ない。** writer 主導（作者がレビューを依頼する形）と比べても差は `main` の 2 往復ぶんしかなく、**本当の分岐は「全文を通すか場所を通すか」だった。** 併せて `main` を通す理由（観測・計数・介入・裁定）を残した |
| 8 | **§4.5.3 進捗の伝え方を新設した** | 毎手順で `project-manager` を起動する案を、**起動の下限（27k〜35k）を根拠に退けた**（概算 3.6M 対 270k）。事実はフック、判断はフェーズ境界に割った |
| 9 | **§4 の全規約表に「なぜ」列を足した** | 規約だけでは移植できない。**環境が変わったとき、何を守るための規約かが分からないと §5 の差し替えができない** |
| 10 | **§1.1 に語の定義を置き、日英の対を固定した** | 生存状態（起動 / 完了 / 待機 / 再開 / 停止）と費用（文脈 / 基盤層 / 会話層 / 下限 / キャッシュ）の語が定義なしに出ていた。**訳語が揺れると日英で別の規則に読める。** 併せて **状態の名前として `中断 / suspend` を使わない**ことを明記した —— **走行を凍らせて後から続ける仕組みは無い** |
| 11 | **`保温` を `キープアライブ`（keep-alive）に置き換えた** | **何を保つのかが語に現れていなかった。** 保つのはエージェントではなく**キャッシュの寿命だけ**である。ウォーム待機 / コールド待機（warm / cold standby）と対で使う |
| 12 | **§1.3 に生存状態とキャッシュの関係を足した** | **「起動と再開がなぜその値段になるか」が本書のどこにも無かった。** §4.1 と §4.4 は、この遷移を知っていることを前提にしている |

### 5.2 節番号の読み替え

**参照している側（`00-mode-matrix.md`・`03-work-order.md`・`06-agent-connection-report.md`）は本版に合わせて直した。**

| 旧 | 新 |
|---|---|
| §1 考え方（§1.1〜§1.8） | **§3 対応方針（§3.1〜§3.8）。枝番は動かしていない** |
| §2 手段（§2.1〜§2.7、§2.5.1・§2.5.2 を含む） | **§4 具体的運用（§4.1〜§4.7、§4.5.1・§4.5.2）** |
| §3 注記 | **§5 注記** |
| （無し） | **§1 背景（§1.1 用語・§1.2 性質・§1.3 生存状態とキャッシュ）・§2 目的・§4.5.3 進捗を新設した** |
| §2.4 保温するか、作り直すか | **§4.4 キープアライブするか、作り直すか**（語を替えた。§5.1 の 11） |
