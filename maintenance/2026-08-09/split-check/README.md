...
**測定日:** 2026-08-09（初回）/ 2026-08-10（骨格に 2 つ目の例を入れて再生成）。**環境:** strictdoc 0.27.1 / Prettier 3.5.3 / Python 3.13.3 / jq 1.8.1 / Windows 11。

**入力は `../03-spec-template-skeleton.md`（骨格）1 枚である。** ここに置いた 20 枚の `.md` はすべてそこから機械で作った。**骨格を直したら、ここも `split-spec.mjs` で作り直す（MUST）** —— 古いままだと、記録が現在の骨格を指さなくなる。**結論と経緯は `../05-step2-split.md` §2.1b にある。**
...
**ファイルをまたぐ親子関係はすべて解決する** —— `SWS-001 → FR-001`（Ch6 → Ch4）、`TC-003 → SWS-001`（Ch10 → Ch6）、`TR-003 → TC-003`（結果の枚 → ケースの枚）。**主張の核である「書き直しが要らない」は成り立っている。**
...
**骨格を 2 件ずつの例に増やしたあと（2026-08-10）、ここも作り直した。** 22 ノードは 9 型を 2 件ずつ持ち、`TC` と `TR` は 3 系統を通した単一連番になっている —— `TC-001` `TC-002` がユースケーステスト、`TC-003` `TC-004` がソフトウェア仕様テスト、`TC-005` `TC-006` が非機能テストである。

**表の桁揃えは骨格と同じ揃えた形である。** 骨格をあらかじめ整形してあるので、**ここに置いた 20 枚は開いて保存しても変化しない**（Prettier 3.5.3 を再適用して差分 0 を確認）。**利用者の VS Code の整形出力は Prettier とバイト単位で一致する**（実測）。
