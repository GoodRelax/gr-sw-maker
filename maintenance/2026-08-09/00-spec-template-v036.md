...
| 9 | **仕様書のノードはレビュー関連の欄を持たない（MUST NOT）。** 下の「レビュー指摘の置き場」を見よ |

**要求ノードの実例（`04-requirements.md` より）:**
...
見出しがそのまま `TITLE` になる。メタデータ欄は `**UID**` の 1 行だけなので行継続の `\` を持たず、その直後に `**Relations**:` が来る。**`**STATEMENT**` のような長い欄は関係の後ろに、空行を挟んで置く。**

**テストケースの実例（`File` 関係つき。`10-sws-test-cases.md` より）:**
...
メタデータ欄が 3 行あるので、最初の 2 行が `\` を持ち、**最終行の `**TEST_LEVEL**` は `\` を持たない。** `File` 関係はテストコードの位置を指し、これがあると「テストケースは書いたがコードが無い」を機械で拾える。

**テスト結果の実例（`10-sws-test-results.md` より）:**
...
**EVIDENCE**: ci/run-4821/junit.xml#test_query::test_invalid_word
**Relations**:
- **Type**: `Parent` \
  **ID**: `TC-003` \
  **Role**: `ResultOf`
...
**結果をテストケースの欄にせず別ノードにしたのは、書き込みの競合を避けるためである。** test-designer がケースを書き、tester が結果を書く。**同じノードを触らないので、走らせるたびに仕様書が変わることもない。** 履歴も残る —— 2 回目の走行は `TR-004` を足せばよく、上書きしない。
