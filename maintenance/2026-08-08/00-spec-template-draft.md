# ANMS v0.35 — AI-Native Minimal Spec Template
...
> **本軸は安定度であって抽象度ではない。** システム構成は物理的で具体的でありながら、与件であるがゆえに最も安定している。**抽象度は別の軸であり、その階段は Chapter 3 で定義する。**
...
> **Chapter 2 のオーナーの例外。** Chapter 2 のオーナーは srs-writer である。**ただし構成を選べる場合、選定は architect が Chapter 5.6 の ADR で行い、確定した構成を srs-writer が Chapter 2 へ反映する。オーナーは移らない。**
...
| 2   | **System Configuration**   | システム構成        | Mermaid + テーブル                  | 最も安定（与件）         |
| 3   | **Use Cases**              | ユースケース        | テーブル（Cockburn brief）          | 安定                     |
...
| --- | --- | --- | --- | --- |
| `GOAL` | Goal | 達成すべき状態 | **根。親を持たない** | Ch1.3 |
| **`ND`** | **Node** | **機械。** PC・サーバー・端末・車載機器など | 鎖の外 | Ch2.2 |
| **`CN`** | **Connection** | **経路。** 対象ソフトが載る機械に出入りするもの | 鎖の外 | Ch2.3 |
| `UC` | Use Case | アクターの目標 | `GOAL` | Ch3.2 |
| `FR` | Functional Requirement | 機能要求 | `UC` | Ch4.1 |
| `NFR` | Non-Functional Requirement | 非機能要求 | `GOAL` | Ch4.2 |
| `ADR` | Architecture Decision Record | 設計判断 | 鎖の外 | Ch5.6 |
| `SC` | Scenario | 受入シナリオ | `FR` | Ch6.1 |

**親をたどると必ず `GOAL` に着く（MUST）。着かないものは Chapter 4.3 の削減候補である。**

**`ND` / `CN` / `ADR` は鎖の外にある。** `ND` と `CN` は NFR の「効く対象」として別の軸で参照され、`ADR` は判断の記録であって要求ではない。

**採番は 3 桁のゼロ詰めとする（MUST）** — `FR-001`、`ND-002`。

**ID を振らないもの:**

| 対象 | 理由 | 章 |
| --- | --- | --- |
| アクター | ユースケース表が名前で参照する。ID を足しても参照経路は増えない | Ch3.1 |
| コンポーネント | 設計をトレースの対象にすると層が 1 つ増える | Ch5.2 |
| 人（アクター）と機械の間の線 | 機械ではないので `ND` を持たない。**ただし経路には `CN` を振る** | Ch2.1 |

**接頭辞を増やすなら、本表に足す（MUST）。** 表に無い接頭辞を本文で使ってはならない（MUST NOT）。**何を指すのかが読み手ごとに変わる。**

> **紐づけの向きは Chapter 4 の「トレースの向き」にある。** 本節は語の定義だけを与える。
...
| --- | --- | --- |
| 利用者 | 主アクター | 相手のアクター |
| `ND-001` 利用者の端末 / `ND-002` サーバー | ノード | 相手の機械 |
| `CN-001` / `CN-002` | 接続 | 相手の経路 |
| `GOAL-001` / `UC-001` / `FR-001` / `NFR-001` / `SC-001` | ID の採番 | 相手の採番 |
| 「知りたいことを問い合わせる」 | ユースケースの目標 | 相手の目標 |
| 単一ノードの CLI の例（§2.1 の 2 枚目） | **最小の構成を示す別の例** | 通し例とは別物。**ID は本例限り** |

**読み替えないもの:** MUST / MUST NOT / SHOULD の規則、章の構成、ID の接頭辞（`GOAL` / `ND` / `CN` / `UC` / `FR` / `NFR` / `SC`）。
...
| 1.3     | Goals       | 目標     | 成功の定義。達成すべき状態。**各目標に `GOAL-xxx` の ID を振る。機能の目標だけでなく、品質の目標（速さ・安全・可用性）も書く** — Chapter 3 のユースケースと Chapter 4.2 の NFR が、どちらもここへ紐づく |
...
| 1.9     | Notation    | 表記規約 | RFC 2119/8174 準拠。主要キーワード例: SHALL/MUST=必須, SHOULD=推奨, MAY=任意。**RFC 8174 は大文字のときだけ規範語とする。EARS 構文中の小文字 `shall` を `SHALL` と同じ拘束力で扱うのは本文書が置く明示の例外であり、その旨を 1.9 に書く（MUST）。** 下の文体規則を含む |
...
> **15 行を超える図は、別ファイルへ出して本文から参照する（MUST）。**

**ANMS は「1 コンテキストウィンドウに収まる」ことを定義に含む形式である。** 図が本文に埋まると、**その仕様書を読むすべてのエージェントが、毎回その全行を読む。** 15 行の図が 10 個あれば 150 行が固定費になる。

| # | 規則 |
|:-:|---|
| 1 | **15 行を超える図は別ファイルへ出す（MUST）。** 置き場は `_assets/` とする |
| 2 | **外に出したら、前後の地の文も直す（MUST）。**「下の図のとおり」が宙に浮く。**これは機械では検出できない。目で読む** |
| 3 | 15 行以下の図は本文に置いてよい |

**閾値は 15 行である。** 図 1 つが画面 1 つに収まる大きさであり、**それを超えると読み手は図と本文を往復し始める。**

---

### Chapter 2. System Configuration (システム構成)
...
```mermaid
flowchart LR
    Operator["利用者"] -->|"CN-001 換算したい値と単位を渡す"| Machine
    Machine["ND-001 利用者のPC<br/>対象ソフトが載る"]:::target -->|"CN-002 換算結果を返す"| Operator
...
**ノードは 1 つ、接続は 2 件である。** 標準出力・標準エラー出力・呼び出し元シェルは同じ機械の内側にあり、ノードを増やさない。**接続が 2 件あるのは規則 7 による** — 対象ソフトは ND-001 に載っており、利用者との出入りがその境界を越える。**利用者が渡す値は入力検証の対象であり、経路を書かなければ検証の起点が消える。**
...
| 4 | **ノードが 1 つでも描く（MUST）** |
| 5 | **図と表を ID で対応させる（MUST）。** ノード（Node）は `ND-xxx`、接続（Connection）は `CN-xxx` |
| 6 | **`ND-xxx` は機械にのみ振る（MUST）。** 人は図の外周に描いてよいが ID を振らない。アクターの定義は Chapter 3.1 が持つ |
| 7 | **`CN-xxx` は、対象ソフトが載るノードに出入りする経路すべてに振る（MUST）。** 相手が機械でも人でもよい。**人に `ND` は振らないが、経路には `CN` を振る** — この経路が入力検証の対象だからである |
| 8 | **対象ソフトが載らないノードどうしの線には `CN` を振らない。** 責任範囲の外である |

#### 2.2 Nodes (ノード)

**ノード表のテンプレート:**

| ND-ID | ノード名 | ノード種別 | 対象ソフトが載るか | 供給元 | こちらで変えられるか |
| ----- | -------- | ---- | ------------------ | ------ | -------------------- |
| ND-001 | 利用者の端末 | 汎用端末 | 載らない | 既存 | 変えられない（利用者の所有物） |
| ND-002 | サーバー | サーバー | **載る** | 新規 | 変えられる |

**供給元は「既存」か「新規」の 2 値である（MUST）。他の値を使ってはならない（MUST NOT）。** 所有者や貸与条件は「こちらで変えられるか」欄に書く。 既存のノードは制約であり、新規のノードは調達・構築のコストを伴う。両者を区別しないと、見積もりも変更の影響分析も成立しない。
...
**人が対象ソフトへ直接与える経路も 1 行として記す（MUST）。** `from` 欄にはアクター名を書く（`ND` は振らない）。**これを省くと、単一ノードの構成で入力検証の対象が 1 件も立たなくなる。**

| CN-ID | from | to | 運ぶもの | 方式 | 入力として信頼できるか |
| ----- | ---- | -- | -------- | ---- | ---------------------- |
| CN-001 | 利用者 | ND-001 | 換算したい値と単位 | コマンド行 | **信頼できない**（利用者が組み立てて渡す） |

上は単一ノードの例（§2.1 の 2 枚目）に対応する。
...
**アクターの視点で、誰が何を達成するかを書く。** Chapter 4 の要求と Chapter 6 の Gherkin はどちらもシステムを主語とするため、**アクターの目標を書く場所は本章だけである。**
...
| 層 | 章 | 主語 | 1 件が表すもの | 例 |
| --- | --- | --- | --- | --- |
| 目標 | Ch1.3 | 利用者 | 達成すべき状態 | `GOAL-001` 利用者が、知りたいことを調べるために作業を中断しない |
...
| Level 3 | 拡張の条件 | **§3.3 で既定として書く** |
| Level 4 | 拡張の処理手順 | **§3.3 の契機があるときだけ書く** |
...
| 同書の部分 | 本テンプレートでの居場所 |
| --- | --- |
| The Use Case as a Contract for Behavior | **持たない。** 本テンプレートは契約をアクターの目標と要求に分けて表す |
| Scope | **Chapter 2 と Chapter 1.5。** 設計スコープは Chapter 2、機能スコープは Chapter 1.5 が持つ |
...
| Linking Use Cases | **持たない。** ユースケースの入れ子を作らない（§3.2 のレベルを `sea` に限る理由） |
| Use Case Formats | **brief だけを採る。** casual と fully dressed は採らない |
...
**表（§3.2）と散文のどちらで書いてもよい。表を既定とする** — 紐づけ先（GOAL / FR）を列で持てるためである。**散文で書く場合も、紐づけ先を同じ枠内に必ず書く（MUST）。** 書かないと Chapter 4.3 の削減候補の検出が入力を失う。
...
| アクター | アクター種別 | 対応するノード | 関心 |
| -------- | ------------ | -------------- | ---- |
| 利用者 | 人 | 該当なし | 知りたいことを、待たずに得たい |

**アクター種別は「人」と「外部システム」を区別する。** 定期実行の基盤や、対象ソフトを呼び出す別のシステムもアクターである。**人だけとは限らない。**

**外部システムのアクターは、Chapter 2.2 のノードとして必ず現れる。** そのときは「対応するノード」欄に `ND-xxx` を書く（MUST）。**書かないと、同じ実体が Chapter 2 と Chapter 3 に別名で並び、影響分析がまたげない。** 人のアクターは機械ではないので「該当なし」と書く。
...
| レベル | 同書での意味 | 本テンプレートでの扱い |
| ------ | ------------ | ---------------------- |
| kite | 複数の利用者目標をまたぐ業務の流れ | **使わない（MUST NOT）。** 分割する |
| **sea** | **1 回の利用で完結する利用者の目標** | **これだけを使う** |
| fish | 他のユースケースから使われる部分機能 | **使わない（MUST NOT）。** それは Chapter 4 の要求である |

**レベルは `sea` の 1 値しか取らない。** それでも列を置くのは、**判定の道具として使うため**である — kite に見えるなら粒度が粗すぎ、fish に見えるなら粒度が細かすぎる。
...
| # | 規則 |
|:-:|---|
| 1 | **1 ユースケースの brief は 1 文で書く（MUST）。** 表なら 1 行、散文なら 1 文である |
| 2 | **表には 1 文の brief だけを書く。** 主成功シナリオの展開と拡張は §3.3 に書く |
| 3 | **レベルは `sea` のみである（MUST）。** kite / fish を書いてはならない（MUST NOT） |
...
#### 3.3 Scenarios and Extensions (シナリオと拡張)

**すべてのユースケースについて、主成功シナリオを 3〜9 手順へ展開し、拡張の条件を書く（MUST）。**
...
|---|---|
| 主成功シナリオ 手順 4 | `FR-001` |
| **拡張 `2a`** 扱えない形 | **`FR-005`（Unwanted Behavior）** |
| **拡張 `3a`** 該当が無い | **`FR-003`** |
| 拡張 `3b` 一部しか得られない | **生まれない。** 利用者への約束ではなく、実現側の都合である |
...
| # | 規則 |
|:-:|---|
| 1 | **主成功シナリオは 3〜9 手順で書く（MUST）。** 9 手順に収まらない目標は `sea` ではなく `kite` の疑いが強い（§3.2） |
| 2 | **番号は `3a.` が条件、`3a1.` `3a2.` が処理手順である。** 処理手順が 1 つで済むなら `->` で 1 行に書いてよい |
| 3 | **条件も処置も完全な文で書く（MUST）。** Chapter 1.9 の文体規則は拡張にも効く |
| 4 | **拡張も手段独立テストに従う（MUST）。**「接続が切れた」は書けるが「BLE が切断した」は書けない |
| 5 | **拡張から生まれた要求には、どの拡張から来たかを 1 行残す（MUST）** |

**処理手順を複数へ割る（`3a1.` `3a2.`）のは、必要になったときだけでよい。** Cockburn の精密度で言えば Level 4 であり、**既定は Level 3（条件と 1 行の処置）までである。**
...
> **Chapter 6.1 のシナリオは、拡張を直接参照しない。** 拡張から生まれた要求を経由して `SC -> FR -> 拡張` と辿れる。
...
| 4.3     | Reduction Candidates  | 削減候補   | 紐づけ先を持たない要求の一覧と、ユーザーの判断 |

#### トレースの向き

> **すべての要求は、`GOAL` を根とする 1 本の鎖に載る。**

**鎖の形:**

```text
SC-001  ──▶  FR-001  ──▶  UC-001  ──▶  GOAL-001
                          NFR-001  ──▶  GOAL-001
```
...
**NFR はもう 1 つ、別の軸の欄を持つ（MUST）。**

> **「効く対象」— どのノード（`ND`）・どの接続（`CN`）に効くか。**

**これは親子関係ではない。** 鎖は `GOAL` に収束し、`ND` / `CN` は「その要求が物理的にどこへ当たるか」を示す。**セキュリティ設計はこの欄を起点にする。**

**各要求は最低限、ID と親の欄を持つ（MUST）。他の列はプロジェクトが決めてよい。**

**紐づけ先が空の要求は Chapter 4.3 に挙げる（MUST）。** ユーザーが「残す」と決めた要求は、**Chapter 4.3 の行が紐づけ先の代わりの根拠となり、欄は空のままでよい。** 空欄をそのまま放置してよいのは、この 1 経路だけである。

#### 4.1 Functional Requirements (機能要求)

**機能要求表のテンプレート:**

| FR-ID | 要求文 | 関連 UC | 出どころ |
| ----- | ------ | ------- | -------- |
| FR-001 | [System] は、[Response] すること。 | UC-001 | 主成功シナリオ 手順 4 |
| FR-003 | もし [Trigger] ならば、[System] は、[Response] すること。 | UC-001 | 拡張 `3a` |
| FR-005 | もし [Trigger] ならば、[System] は、[Response] すること。 | UC-001 | 拡張 `2a` |
| FR-010 | [System] は、[Response] すること。 | UC-002 | 主成功シナリオ |

> **型（Ubiquitous / Event-driven 等）を列に持ってはならない（MUST NOT）。** 型は要求文そのものに現れる。**型名を別の欄に書くと、要求文を書き直したときに欄が古いまま残り、どちらを信じるかを決められなくなる。**
...
**`[System]` の定義。** EARS の `[System]` は、**Chapter 2.2 で「対象ソフトが載る」と記したノード上で動くソフトウェアを指す。** Chapter 2 を書かずに Chapter 4 を書いてはならない（MUST NOT）。主語が定義されないまま要求を書くことになる。

#### 4.2 Non-Functional Requirements (非機能要求)

**非機能要求表のテンプレート:**

| NFR-ID | NFR分類 | 要求文 | 関連 GOAL | 効く対象 |
| ------ | ---- | ------ | --------- | -------- |
| NFR-001 | 性能 | [System] は、[Response] すること。 | GOAL-001 | CN-001, CN-002 |
| NFR-005 | セキュリティ | もし [Trigger] ならば、[System] は、[Response] すること。 | GOAL-002 | CN-001 |
| NFR-006 | 正確性 | [System] は、[Response] すること。 | GOAL-001 | ND-002 |

**`関連 GOAL` が親であり、`効く対象` は別の軸である**（§トレースの向き）。**GOAL-002 は品質の目標**（例: 利用者が渡した値によってシステムが壊れない）であり、Chapter 1.3 に書く。
...
> **判定は 1 つである — 辿っていって `GOAL` に着かないものは削減候補である。**

| 対象 | 親 | 候補になる条件 |
| ---- | -------- | -------------- |
| ユースケース（Ch3.2） | `GOAL` | どの目標も満たさない |
| FR | `UC` | どのユースケースにも属さない（**その先で `GOAL` に着かない**） |
| NFR | `GOAL` | どの目標も満たさない |

**鎖が 1 本なので、判定も 1 つで済む。** 途中のどこで切れていても、根に着かないという 1 つの条件で拾える。

**あわせて、別の軸でも 1 つ拾う。**

| 対象 | 条件 |
| --- | --- |
| NFR | **対象ソフトが載らないノードだけを「効く対象」にしている**（責任範囲の外である） |
...
| 5.2     | Components           | コンポーネント       | 常に     | 部品と責務の分割。コンポーネント図（5.1の凡例で色分け）。**各コンポーネントがどのノード（ND-xxx）に載るかを書く。** AI/LLM連携がある場合はプロンプトテンプレートの配置（`src/prompts/` 等）・入出力スキーマ・テスト方針・ハルシネーション対策も定義する |
...
| 5.5     | Behavior             | 振る舞い             | 常に。**ただし図は種類ごとに条件が異なる** | 処理フロー・相互作用。シーケンス図（**複数コンポーネントの相互作用がある場合**）、アクティビティ図（**分岐・並行が多い場合**）。**ノードをまたぐ相互作用は、Chapter 2.3 の接続 ID（CN-xxx）を明記する** |
| 5.6     | Decisions            | 設計判断             | 常に     | ADR（Architecture Decision Records）。判断理由・代替案・決定者。記録形式は Michael Nygard の ADR フォーマット（Title / Context / Decision / Status / Consequences）を推奨。**ADR-000「最小構成との比較」を必ず含める**（要求を満たす最小の構成、採用案が増やした要素、各々を増やした理由。R2.18）。**キャッシュを用いる場合はキャッシュ方針の ADR を含める**（R2.20）。**システム構成を選べる場合、その選定を ADR に記録する**（Chapter 2） |

**コンポーネントとノードの対応。** Chapter 5.2 は、**各コンポーネントが Chapter 2.2 のどのノードに載るかを書く（MUST）。** 単一ノードなら「すべて ND-001 に載る」の 1 行で足りる。これを書かないと、物理構成の章と論理構造の章が互いを参照せずに離れていく。
...
**シナリオは `UC-xxx` を直接参照しない。** FR が UC を持つため、`SC -> FR -> UC` の順に辿れる。参照を二重に持たせると、改訂のたびに両方を直すことになる。**Chapter 3.3 の拡張から生まれたシナリオも同じである** — 拡張が生んだ `FR` を参照すれば、そこから拡張へ辿れる。
...
Feature: [機能名]
  ...
  Rule: [ビジネスルール名]
    ...
    Scenario: SC-901 [シナリオ名] (traces: FR-xxx)
      Given [前提条件]
      And [追加の前提条件]
      When [操作・イベント]
      Then [期待結果]
...
```gherkin
Scenario: SC-902 [シナリオ名] (traces: FR-xxx)
  Given [前提条件]
  When [操作・イベント]
  Then [期待結果]
```
...
| 性能テスト   | [対象API/処理]       | Chapter 4 NFR の数値目標に基づく。**対象の接続 ID（CN-xxx）を明記する** | [例: k6]              | [目標値]         |
| E2Eテスト    | [主要ユーザーフロー] | Chapter 6.1 Gherkin シナリオに対応 | [例: Playwright]      | 全シナリオPASS   |

**ノードをまたぐテストの明示。** 複数のノードにまたがる検証は、**どの接続（CN-xxx）を実際に通すかを書く（MUST）。** 書かないと、実機で通すのか模擬で代替するのかが実装者の判断に委ねられ、後から再現できない。
...
| **System Configuration を Requirements の前に置く** | **EARS は全要求に `[System]` を強制する。その `[System]` が何であって何でないかを、要求より先に定める必要がある。** 与件（車両・端末・サーバー）は設計判断より安定するため、上位に置く |
| **構成図で対象ソフトのノードを太枠で示す** | **色は Ch5.1 の層凡例に予約されている。** 同じ図法で違う意味を持たせると読み手が混同する |
| **持たないものを Ch2.4 に明記**       | **攻撃面の列挙の起点である。** 信頼境界が Architecture 章まで現れないと、要求を書く間これを持たないまま進むことになる |
| **ノードが 1 つでも構成図を描く**     | 省略を許すと「外部依存が無い」ことが暗黙になり、Ch2.4 の判断が根拠を失う |
| **構成の選定は Ch5.6 の ADR へ逃がす** | 与件の章に判断を混ぜると、「変えられないもの」と「選んだ結果」を後から区別できない |
| **Use Cases を Requirements の前に置く** | **Ch4 の EARS も Ch6 の Gherkin もシステムを主語とする。** 本章が無いと、仕様書はアクター視点の記述を 1 つも持たない |
| **拡張を既定で書く**                  | **拡張こそが要求を生む。** 主成功シナリオだけでは、望ましくない挙動型の要求がまるごと抜ける。**歯止めとして「拡張の数だけ要求を作らない」を併記する** — 実現側の都合にすぎない拡張がある |
| **Level 4（処理手順の分割）は契機があるときだけ** | 同書の「幅優先で低い精密度から上げる」に従う。**条件（Level 3）を書かないと要求が生まれないが、処置の分割は後から足せる** |
| **抽象度の階段を Ch3 冒頭に明記**     | **ユースケースが要求と同じ粒度に落ちると Ch4.3 の削減候補検出が働かない**（すべての要求が自動的に紐づけ先を得る）。目的と同じ粒度なら Ch1.3 の写しになる。**手段独立テストと粒度の判定で位置を固定する** |
| **STFB の軸は安定度であって抽象度ではない、と明記** | **システム構成は具体的でありながら、与件ゆえに最も安定している。** 安定度と抽象度を 1 本の軸にまとめると、Ch2 の位置が説明できない |
...
| **EARS の型を列に持たない**           | **型は要求文そのものに現れる。** 型名を別の欄に置くと、要求文を書き直したときに欄が古いまま残り、**どちらを信じるかを決められない** |
...
| **15 行を超える図を別ファイルへ出す**  | **ANMS は「1 コンテキストウィンドウに収まる」ことを定義に含む。** 図が本文に埋まると、読むエージェント全員が毎回その全行を読む |
| **記入例の読み替え表を置く**          | 記入例が固有名詞を含む以上、**どこが例でどこが規則かの線を引かないと、読み手が題材ごと真似る** |
| **目標レベルは記号でなく語で書く**    | 同書は雲・凧・波・魚・貝の記号と `+` `!` `-` の印を示すが、**表で扱えて検索できるのは語である** |
| **Ch2 の粒度を「機械」に固定**        | 標準出力・標準エラー出力・呼び出し元シェルは**同じ機械の内側にあり、機械の数を増やさない。** これらをノードとして描くと、概要図の粒度が OS の入出力経路まで落ちる |
| **図と表を ND / CN の ID で対応させる** | **NFR が CN-xxx を指す以上、図のどの線を指しているかが特定できなければトレースが成立しない。** 図と表に同じノードが出るだけでは、対応する手段を持たない |
| **接続は向きごとに 1 行**             | 「入力として信頼できるか」は送る向きと受け取る向きで判定が変わる。1 行にまとめると、どちらの判定なのかが読めない |
| **通し例を 1 つに固定する**         | **章ごとに別の題材を使うと、同じ ID が別のものを指し、章ごとに抽象度が違うようにも見える。** 例は「利用者がサーバー上のシステムへ問い合わせ、結果を受け取る」に統一する — ノード 2・接続 2・アクター 1 で、全部の表の形が見える最小である。**単一ノードの例だけは別に置き、その ID は本例限りと明記する** |
...
| **文体規則は記述文にのみ適用する**    | **規則文は「〜する（MUST）」の形で主語を省くのが正しい日本語である。** 指示文にまで主語を強制すると、規則そのものが読みにくくなり、規則を守る文書のほうが読めなくなる |
...
| **鎖を `GOAL` 1 本に収束させる**       | 根が 1 つなら、**削減候補の判定が「辿って根に着くか」の 1 条件で済む。** 2 本に割れていると、要求の種類ごとに別の判定が要る |
| **NFR の親を `GOAL` にする**          | 性能・安全・可用性は横断的でアクターの目標に属さないが、**「どの目標のために速く / 安全であるべきか」は必ず書ける。** 書けない NFR は誰のためのものでもない。**そのために Ch1.3 に品質の目標を書かせる** |
| **`ND` / `CN` を親子関係にしない**    | **物理的にどこへ当たるかは、抽象度の階段とは別の軸である。** 親にすると鎖が 2 本に割れる。**効く対象として別の欄に置き、セキュリティ設計の起点にする** |
...
| Gherkinの主要キーワードを提示           | Feature, Background, Rule, Scenario, Given/And/When/Then/But をテンプレートで示す。**Scenario Outline / Examples / Doc Strings / Data Tables / Tags / Comments は載せていないが、必要なら使ってよい**（Gherkin Reference に従う） |
