...
| 1.3     | Goals       | 目標     | 成功の定義。達成すべき状態。**各目標に `GOAL-xxx` の ID を振る**（Chapter 3 のユースケースが参照する） |
...
> **拡張を書き足すユースケースは、そのときに限り主成功シナリオを 3〜9 手順へ展開する（MUST）。** §3.2 の brief は 1 文なので手順番号を持たず、**展開しないまま `3a.` の「3」を決められない。**
>
> **これは Cockburn の精密度を Level 2 から Level 3 へ上げる操作そのものである。** 展開するのは拡張を持つユースケースだけで、他はすべて 1 文のまま据え置く。
...
| 5 | **番号は同書の流儀に従う。** 展開した主成功シナリオの手順番号 + 英小文字（`3a.`）で条件を書き、処理手順（`3a1.` `3a2.`）を続ける |
...
**各要求は最低限、ID と紐づけ先の欄を持つ（MUST）。他の列はプロジェクトが決めてよい。**
...
| NFR | ND-xxx / CN-xxx | どのノード・接続にも効かない。**または、対象ソフトが載らないノードだけを指している**（責任範囲の外である） |
...
**シナリオは `UC-xxx` を直接参照しない。** FR が UC を持つため、`SC -> FR -> UC` の順に辿れる。参照を二重に持たせると、改訂のたびに両方を直すことになる。**例外は Chapter 3.3 の拡張から生まれたシナリオで、その場合のみ Remark に「UC-001 の拡張 3a に対応」と記す。**
