...
計画の 6-1 〜 6-55 のうち **53 項目**を適用した。うち 9 項目は Step 0 / Step 3 / Step 5 で前倒し実施した（6-29 〜 6-33, 6-38, 6-39, 6-45, 6-46）。

> **本節の当初の記述「55 項目はすべて適用済み」は誤りであった。** 実際には 6-50 と 6-52 が未実施のまま残り、6-51 の内容が「6-50」として記録されていた（番号が 1 つずれ、末尾 2 項目が脱落した）。Step 6d で解消した。誤りを残さないため本文を訂正する。

---

## Step 6d — 未実施項目の解消と R7 の配線完了

Step 7 着手前の現物確認で、Step 6 に 2 項目の脱落が判明した。いずれも後続の前提であるため先に処理した。

| # | 内容 | 実測した状態 |
|:-:|---|---|
| 6-50 | `/full-auto-dev` への 17 体分の明示的な名指し起動 | **未実施。** 名指しは 8 体のみで 14 体がゼロ |
| 6-52 | フェーズ完了時に `session-state.json` → `cost-log.json`、閾値到達で handoff | **未実施。** `session-state` / `cost-log` の記述が 1 件もない |

**なぜ先に処理したか:** 6-52 は 7-2（`session-meter.mjs`）の唯一の消費者である。これを欠いたまま計測器を作ると、書き手だけいて読み手がいない write-only ファイルになり、C10-d で削除した `form_block_cardinality` と同型になる。6-50 は議題2の中核であり、これがないと `technical-authority` が一度も起動されず議題3のゲート設計が空転する。

### 6d-1 名指し起動と計測の配線（`0e982b0`）

| 項目 | 内容 |
|---|---|
| 6-50 | 全フェーズの手順に owner エージェント名を付与。ja/en とも **22/22 体**が名指しされる状態にした |
| 6-52 | 「各フェーズ完了時の共通手順」節（Fa-Fg）を新設。kotodama-kun / progress-monitor（コスト追記）/ handoff / orchestrator / process-improver / decree-writer / change-manager |

**計画との差分:**

| # | 差分 | 理由 |
|:-:|---|---|
| 1 | 「17 体」→ 13 体 + 対象外 1 体 | 計画時点は 21 体・名指し 4 体。現在は 22 体・名指し 8 体で母数が違う。`framework-translation-verifier` は In が `framework-src/{ja,en}/**` であり、ユーザープロジェクトに検証対象が存在しないため名指しせず、「本パイプラインでは起動しない」と明記した |
| 2 | `3g2.`（deployment-design の作成）を追加（計画外） | GATE-DESIGN が「deployment-design が存在」を要求する一方、それを作る手順がどこにもなかった。**満たしようのないゲート**だった。同じ理由で `4c2.`（implementer が infra/ を実装）も追加 |
| 3 | CLAUDE.md「品質目標」に「コンテキスト使用率の引継ぎ閾値」行を新設（計画外） | プロセス規則 §3.2.7 が「CLAUDE.md の閾値」を参照するのに、CLAUDE.md に該当行が存在しなかった。6-52 が参照先を持てない |
| 4 | `5c2.`（実機テスト系 3 体の起動）を追加 | Phase 0 の `0n2.` が「testing フェーズでアクティベートする」と書きながら、Phase 5 に該当手順がなかった |

### 6d-2 R7 の配線完了（`594615c`）

**発見:** R7 は review-standards で定義され、review-agent に配線され、`review` file_type にも追加済みだったが、**どのゲートも R7 の PASS を要求していなかった。** 下流の記述がすべて R7 以前の観点集合のままだった。

| 記述 | 実測 |
|---|---|
| `R2/R4/R5` / `R2/R3/R4/R5` / `R3/R5` / `R2,R4,R5` の残存 | **36 箇所 × 2 言語 = 72 箇所**（9 ファイル） |

9-8 の `grep 'R1-R6'` はこの形を検出できないため 0 件で通っていた。**「検査が通った」ことは「配線が完了した」ことを意味しない。** 4a のサブラベル見落とし（Step 5a に記録）と同型の失敗である。

対象は GATE-DESIGN / GATE-IMPL の条件、technical-authority のフェーズ遷移条件・End Conditions・出力例、orchestrator の遷移表、review-agent の In とレビュー出力例、agent-list のアクティベーションマップ、field-issue の状態定義、プロセス規則の Mermaid ゲートフロー・ガント・KPI 表。

`review-agent` の出力例の `dimensions: R2,R4,R5` も `R2,R4,R5,R7` に修正した。文書管理規則 §9.3 の値域例は既に `R2,R4,R5,R7` であり、**出力例だけが R7 を落とす形**になっていた。

GATE-PLANNING（R1）と GATE-TEST（R6）は変更していない。R7 の適用対象は Ch3-4 とコードであり、Ch1-2 とテストコードではない。

### 未処理として送ったもの

`architect:36` と `implementer:34` の End Conditions が「review-agent の R2/R4/R5/R7 レビューに PASS している」となっている。4-5 で test-engineer について解消した B3 型（他エージェントの実行結果を自分の完了条件にすると自力で満たせない）と同型の欠陥が 2 体に残っている。R7 配線とは別テーマのため本 Step では扱っていない。

---

## Step 7 — スクリプト群と CI

**方針:** 「検査が見つける不整合を先に直す → 検査スクリプト → CI → 実行時フック」の順で進めた。CI 導入時点で赤にならないようにするためである。

### 7 着手前の実測（試作した検査で測定）

| 検査 | 結果 |
|---|---|
| 構造パリティ（見出し / 表行 / フェンス） | 40 ファイル**全一致**。ハード FAIL 条件にできる |
| 行数 | 不一致 1 件。`document-rules` の ja で `## 9.33` の前の `---` が欠落 |
| リンク集合 | アンカーとプレースホルダを除けば全一致 |
| 数値トークン | **40 中 35 件不一致。検査に使えない**（en が `OpenAPI 3.0` のように書き分ける） |
| デッドリンク | 26 件（すべて README の旧サフィックスパス） |
| Form Block タグ名 | **9 件が §9 に不在**（ja/en 同一） |

**計画 §9 の「数値トークンの一致」は撤回した。** 実測で恒常的に不一致となり、検査として成立しない。

### 7a 文書管理規則の自己整合（`8cd2df7`）

| 種別 | 件数 | 内容 |
|---|---:|---|
| §4.2 の未定義タグ名 | 9 | `unmitigated_high_count` → `unmitigated_critical_count` 等 |
| 廃止エージェント名 `lead` | 4 × 2 言語 | §4.2 内 3 + §7.1 / §9.4 / §11 |
| §9 に直書きされた閾値 | 7 × 2 言語 | §9.4.1 の GATE ID 参照へ |
| 区切り線 | 1 | ja の `## 9.33` 前 |

**`final-report` の 4 フィールドは追加せず、例を書き換えた。** 求めていた 4 指標は `progress:test_pass_rate` / `coverage_pct` / `defect_open_critical` / `defect_open_high` として既に存在し、owner も progress-monitor である。追加すれば owner の違う 2 箇所に同じ測定値を置くことになる。**「他の file_type が既に持つ測定値は転記しない」という配置判断の実例**に置き換えた。§4.1 は情報の配置基準の章であり、教材としてはむしろ強くなる。

**どちらがドリフトした側かは実測で決着した。** §4.2 は存在しない `lead` エージェントを 3 箇所で主語にしている。§9 は 6-21〜6-37 で保守されてきた。

**閾値 7 件のうち 3 件は遷移先そのものが誤っていた**（`security-scan-report` は GATE-IMPL なのに "delivery transition"、`interview-record` は GATE-INTERVIEW なのに "planning → design"）。§9.0 が「閾値を複数箇所に書くと必ず乖離する」と主張する実例が §9 自身の中にあった。

**この修正で ja/en パリティは全 40 ファイルで不一致ゼロになった**（Step 3 以来報告してきた 2 行差の正体がこの区切り線だった）。

### 7b README のリンク更新（`62b0b29`、8-12 の前倒し）

26 本のデッドリンクを `framework-src/{lang}/process-rules/` へ更新（実置換 28 箇所。2 本はコードスパンのラベル自体がパス）。あわせて README 両言語の「R1-R6」→ R1-R7、README-ja のリンクラベルを実文書題名「実機テスト フィードバック管理規則」に修正。

> **本コミットには 7c の成果物も混入している。** ユーザーが stage 範囲を分けずにコミットしたため、検査スクリプト 4 本・共通ライブラリ・`jsonl2md` 修正・`translate-framework` の `doc:type` 修正が同一コミットに含まれる。内容は正しいがコミット文は README の分しか説明していない。push 済みのため修正には force-push が要る。

### 7c 検査スクリプト（`62b0b29` に同梱）

| ファイル | 検査内容 |
|---|---|
| `tools/lib/framework.mjs` | 共通ヘルパー（言語列挙・Markdown 走査・構造カウント・リンク抽出） |
| `tools/check-parity.mjs` | 言語ツリーの一致、行数・見出し・表行・フェンス・リンク先 |
| `tools/check-roster.mjs` | 名簿と実体、frontmatter の `name` / `model`、レビュー観点の配線 |
| `tools/check-links.mjs` | デッドリンク、節番号参照の実在 |
| `tools/check-tagnames.mjs` | Form Block タグ名が §9 に実在すること |

**計画との差分:**

| # | 差分 | 理由 |
|:-:|---|---|
| 1 | 節番号参照の検査を `check-roster` ではなく `check-links` に置いた | 「参照先が実在するか」という主題が一致する。check-roster は名簿とオーナーシップに純化した |
| 2 | 「`R7.x` 全 ID が review-agent の観点表に出現」を **R レベルの双方向一致**に読み替えた | review-agent は R7 粒度でしか参照していない（規約の重複を避けるため意図的）。文字どおりには成立しない |
| 3 | R 集合の退行ガードを追加（計画外） | `R2/R4/R5` 等が R7 を伴わない記述を FAIL とする。6d-2 で直した 72 箇所の再発を機械で防ぐ |
| 4 | 節番号参照のラベル対応表はスクリプト内に持つ | ラベルは localized で機械導出できない。未登録ラベルは黙って飛ばさず、全規則文書の節番号との照合にフォールバックし警告する |

**負のテスト（壊すと落ちることの確認）:** 見出しの降格 / model 不一致 / frontmatter name 不一致 / フィールド改名 / デッドリンク / 存在しない §番号 / R7 の退行 — すべて検出。

**手戻り:** 負のテストの後始末に `git checkout -- README.md` を使ったが、7b が未コミットであることを確認しておらず変更を消した。同内容を再適用済み。以降は git を使わず元バイト列を保持して復元している。

### 7d CI と pre-commit（`f6b0db6`, `18a5007`）

| ファイル | 内容 |
|---|---|
| `.github/workflows/framework-check.yml` | syntax / parity / roster / links / tagnames / setup の 6 ステップ |
| `tools/check-setup.mjs` | 展開内容・冪等性・言語切替・`.bak` 保護・配布整合 |
| `tools/hooks/pre-commit` | 片側だけの stage を拒否し、続けて check-parity |

**計画との差分:** 計画は「setup smoke」を CI の検査項目として挙げるだけだったが、独立スクリプトにした。**YAML 直書きだと手元で再現できず、「手元で通れば CI も通る」が成立しない。** 副次的に 9-2 と 9-3 が機械化された。一時ディレクトリに複製してから実行するため、作業中の `CLAUDE.md` を壊さない。

**残骸除去の検査は当初素通りしていた。** ja と en はファイル名が同一のため、`setup.js` の残骸除去コードが一度も実行されていなかった。sandbox 内に意図的に構成の異なる言語を作って経路を踏ませるようにした。**踏めない経路は検査していないのと同じである。**

pre-commit は `node` が PATH にない場合、check-parity を落とさず警告して続ける。**フックが全コミットを止める事故のほうが、防ごうとしているドリフトより高くつく。**

`.gitattributes` に `tools/hooks/*` と `*.yml` の `eol=lf` を追加した。フックは拡張子がなく既存ルールに掛からず、CRLF で checkout されると Linux / macOS で `bad interpreter: /bin/sh^M` となり、**インストール済みに見えて何もしないフック**になる。

実行ビットは GUI コミットで一度 100644 に戻され、`18a5007` でコマンドラインから修正した。

### 7e 実行時の機構（`17caaac`）

| ファイル | 内容 |
|---|---|
| `tools/gate-guard.mjs` | `PreToolUse` フック。ゲート未通過の書込みを拒否 |
| `tools/session-meter.mjs` | statusLine。`session-state.json` を更新 |
| `.claude/settings.json` | 上記 2 つの登録 |
| `create-gr-sw-maker/bin/create.js` | `tools/` を許可リスト方式に変更 |

**`create.js` の修正は必須だった。** `FRAMEWORK_ONLY` が `tools` を丸ごと削除しており、**このままだと `gate-guard` と `session-meter` がユーザーに届かず、`settings.json` が存在しないファイルを指す状態で配布される**ところだった。許可リスト方式にしたので、今後フレームワーク用ツールを足しても既定で配布されない。

**計画・決定との差分:**

| # | 差分 | 理由 |
|:-:|---|---|
| 1 | GATE-TEST の対象を `final-report.md` のみにした | 議題3の表は `docs/manual/` を挙げるが、`user-manual` の出力先は `docs/` 直下。`docs/` を守ると planning の仕様書執筆を止める。`docs/operations/` も design フェーズの `deployment-design` があり守れない |
| 2 | `check-setup` に配布整合の検査を追加（計画外） | `settings.json` が参照するツールが `create.js` の許可リストにあり実在することを検査する。この穴は実行時に何も言わずに壊れる |

**gate-guard の負のテスト（19 ケース）:** ゲート未通過の拒否 / 常時許可パス / 非書込みツール / プロジェクト外 / 空入力・非 JSON・`tool_input` 欠落での fail open / 環境変数による無効化 / `result=fail` は開かない / 別ゲートの合格は開かない / `review:gate` 不在時の `dimensions` フォールバック / **R7 を欠く設計レビューでは GATE-DESIGN が開かない** — すべて期待どおり。

**配布シミュレーション:** 生成プロジェクトの `tools/` は 2 本のみ、`settings.json` の参照先 2 つとも解決、framework-only は全消去、`framework-src/` は保持。

---

## Step 8 — README と公開物

### 8a README の全面改稿（`615d1bc`）

| 項目 | 内容 |
|---|---|
| 8-9 | フェーズ表を Phase 0-7 に修正（**従来は 1-8 で全行が 1 つずれていた**）。`Conditional` 列と Phase 1 のモックイテレーションを明記 |
| 8-3 / 8-4 | 移植比率を実測値に置換。未検証であることを明記 |
| 8-11 | 「フレームワーク開発者向け」を `framework-src` 構成に全面改稿。`tools/` の 2 種類とフック有効化を追記 |
| 8-5 | 「Or from GitHub」を撤回 |
| 8-10 / 8-15 | 「それだけ。」を撤回し判断点 5 つを列挙。EN の "Accept the deliverables" を "run the acceptance tests" に |
| 8-1 / 8-2 / 8-6 / 8-7 / 8-8 | See it in Action / Cost / Requirements / Scope and Limitations / Claude Code 起動手順を新設 |

**移植比率の実測:**

| 測り方 | 結果 |
|---|---|
| フォーマット変換が不要な行 | **92.6%**（11,296 行中 10,460 行） |
| ベンダー固有語 | **197 箇所**、移植ガイド自身を除く 39 ファイル中 32 ファイル |
| 何も変更不要な文書 | **4 本** |
| ファイル数で測った場合 | ポータブルは 40 中 9 |

従来の「約70% / 15% / 15%」はどの測り方でも再現しない。**ファイル数は採用しなかった** — 「エージェント定義 1 本の frontmatter 数行の変更」を「1 ファイル全変換」と数えてしまうためである。

**副産物:** 移植ガイドの「変更不要」表が誤っていた。`glossary` / `review-standards` / `prompt-structure` が載っているが計 13 箇所のベンダー固有語を含み、逆に 0 箇所の `field-issue-handling-rules` が載っていなかった。**README が公開する数値と規則文書が食い違う状態は作れない**ため同一コミットで修正した。

作業中に ja だけ更新され en が取り残された瞬間があったが、`check-parity` が `lines ja=218 en=220 / tableRows ja=95 en=97` で即座に検出した。

### 8b エッセイと調査レポート（`cc7994c`）

**8-13:** `update-direction.md` の 6 件の改善案のうち **5 件を適用し、改善1 は適用しなかった。**

| | 内容 |
|---|---|
| 適用しない理由 | 現行エッセイには既に **§1.4「第1論文との関係」**が独立節として存在し、比較表まで備える。しかも両者は逆の立場を採る（改善案「前提の転換」/ 現行「抽象度を上げる拡張」）。§1.4 は改善案より後に書かれた著者の判断であり、適用すれば同一文書内で矛盾する |
| 節番号のずれ | 改善案が指す 2.2 / 2.3 / 2.4 は、§1.4 独立後の現行では 2.1 / 2.2 / 2.3。番号ではなく節の題名で照合した |
| 改善4 の調整 | 原案は CQRS が本文既出の前提だが、**本文には一度も現れない**（Keywords と References にはある）。「本節の責務分離は CQRS[3] の構図に対応する」と導入してから Fowler の警告に応答する形にした |

**`## 7. 限界と今後の課題` を新設し結論を `## 8.` に繰り下げた。** ja/en とも 646 行 / h2 12 / h3 25 / 表行 125 / フェンス 16 で一致。

**8-14:** 改名。どちらの旧名も**どの AI が書いたかを示すだけで内容を示していなかった。**

| 旧 | 新 |
|---|---|
| `claude-chat-review.md` | `angs-prior-art-review-2026.md` |
| `gemini-review.md` | `anms-architecture-review-2026.md` |

後者は ANGS ではなく **ANMS** のアーキテクチャレビューであり、旧名からは判別できなかった。

> **本コミットのメッセージは 1 つ前（README）のものが再利用されており、内容と一致しない。** push 済みのため修正には force-push が要る。
...
| # | 検証 | 結果 |
|:-:|---|---|
| 9-1 | CI 全項目 | **PASS**（syntax 11 本 / parity 40 対 / roster 44 定義 / links 116+268 / tagnames 78 / setup 2 言語） |
| 9-2 | 言語切替の残骸 | **PASS**（check-setup が機械化） |
| 9-3 | `.bak` 保護 | **PASS**（`--force` で退避しないことも確認） |
| 9-4 | ゲート未通過の `src/` 書込み拒否 | **PASS（実セッションで実証）** |
| 9-5 | `session-state.json` の生成 | **FAIL** |
| 9-6 | framework-src 内の相互リンク | **PASS**（82 参照 0 デッド。GitHub 上でも 200） |
| 9-7 | `angs-essay-en.md` のレンダリング | **PASS**（六重バッククォート 0、フェンス 16 で偶数） |
| 9-8 | R7 の配線 | **PASS**（`R1-R6` 残存 0、ゲート条件 4 箇所が R7 を要求） |

### 9-4 — 未検証事項が解消した

`src/` への書き込みを実際に試み、フックが拒否した。`src/` は空のままでファイルは作られていない。

**`$CLAUDE_PROJECT_DIR` は Windows でも展開され、`.claude/settings.json` は再起動なしで読まれる。** 7e で「未検証」として報告した事項は解消した。

### 9-5 — 計測経路が働かない環境がある（`e762fdc`）

フックは動作するのに `session-state.json` はマシン上のどこにも作られていない（全走査で 0 件）。本セッションの入口は `CLAUDE_CODE_ENTRYPOINT=claude-desktop`。**statusLine はステータス行を描画する環境でしか実行されない。**

議題8はこの経路を「モデルが自分のトークン消費を観測する唯一の手段」として設計している。働かない環境では `cost-log.json` は空のまま、コンテキスト閾値の判定も成立しない。**しかもどの消費者も「ファイルが無い場合」を規定していなかった。**

> **ターミナル CLI で動くかどうかは検証できていない。** 動くと仮定した記述はしていない。

**対処（欠測の可視化）:**

| 対象 | 修正 |
|---|---|
| `session-meter.mjs` | 書込み失敗時に `meter: NOT RECORDED` を表示。**従来は書けなくても正常に見えていた** |
| `/full-auto-dev` Fb / Fc | ファイル不在時の分岐。「計測不能を記録して報告する。**推測で書いてはならない（MUST NOT）**」 |
| `progress-monitor` Exception | `session-state.json が存在しない` の行を追加 |
| プロセス規則 §3.2.7 | 計測経路が働かない場合があること、欠測を明記すること |
| 移植ガイド / README 両言語 | 同じ but 書きを追加 |

**この発見は検査の副産物ではなく、実機で踏んで初めて出た。** 静的解析では「statusLine に登録した」で完結して見える。

### 13.2 ユーザー側の作業（リポジトリ修正では完結しない）

| # | 内容 | 状態 |
|:-:|---|---|
| 1 | `create-gr-sw-maker` の再 publish | **未実施。必須。** `create.js` の修正（配布範囲・ダウンロード処理・`--ref`）はパッケージの中身そのもの |
| 2 | `npm deprecate gr-sw-maker "Use: npm init gr-sw-maker"` | **未実施。** 72 時間超過のため unpublish は不可。漏洩がないことは確認済み |
| 3 | `gr-sw-maker-examples` の整備 | **未実施。** README から「実際に作られたもの」節でリンク済みのため、ToC の整備が先に見える |

### 既知の未処理

| # | 内容 | 扱い |
|:-:|---|---|
| 1 | `62b0b29` と `cc7994c` のコミット文が内容と一致しない | push 済み。修正には force-push が要る |
| 2 | `architect` / `implementer` の End Conditions が他エージェントの実行結果を条件にしている | B3 型。別テーマ |
| 3 | `essays/research/*.md` に `R1-R6` が 8 箇所 | 調査レポート。04-work-plan §14 が別テーマとしている |
| 4 | `framework-src/ja` の散文に「フィールドテスト」が 5 箇所 | 用語集がどちらも定義しておらず正本の判断が要る。kotodama-kun 案件 |
| 5 | 日付書式の不統一 | 同上 |
