...
|----------|---------------|:----------:|
| プロセス規則 | `process-rules/*-ja.md` ↔ `*-en.md` | 9 |
| エージェント定義 | `.claude/agents/*-ja.md` ↔ `*-en.md` | 18 |
...
| F11 | `.claude/agents/*-en.md`（18本） | エージェント定義 |
...
**起動方法:** `Agent tool`（description: "Prompt quality check for 18 EN agents", run_in_background: true）

**プロンプト（以下をそのまま渡す）:**

```
gr-sw-maker フレームワークの全18エージェント定義ファイル（EN版）の品質チェックを行う。
...
EN 版ファイルを主対象とする（CLAUDE.md は言語バリアントなし）。

## チェック項目

### C1: エージェント数の一致
全ファイルで「18」（エージェント数）に関する記載を検索し、異なる数値がないか確認する。
対象: CLAUDE.md, process-rules/*-en.md, essays/anms-essay-en.md, essays/angs-essay-en.md
