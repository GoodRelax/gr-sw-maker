...
- `tools/otel-sink.mjs` runs as an OpenTelemetry receiver and writes cost and tokens to `project-management/progress/session-state.json`
...
- **Telemetry is dropped in silence while the receiver is not running.** The file therefore carries a freshness marker, and the process records the gap rather than estimating around it

**What you have to set**

`CLAUDE.md` "Quality Targets" holds the cost budget and its alert threshold. Both are required entries. Left as placeholders, the alert has nothing to compare against and never fires.
...
- **`tools/gate-guard.mjs`, `tools/otel-sink.mjs` and `tools/session-meter.mjs` are Claude Code specific.** Dropping them is supported; gate enforcement and cost alerting then rely on manual checks
...
- **`tools/` holds two different kinds of script.** `check-parity`, `check-roster`, `check-links`, `check-tagnames`, `check-terms`, `check-setup` and `jsonl2md` exist to guard this repository and run in CI. `gate-guard`, `otel-sink` and `session-meter` run inside a user project, and are the only three that `create.js` ships.
