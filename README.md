...
- **Source files** use language suffixes: `*-ja.md` (Japanese) and `*-en.md` (English). These are the originals tracked by git.
- **`node setup.js <lang>`** copies the selected language version to a suffix-less file (e.g., `CLAUDE-ja.md` → `CLAUDE.md`). These suffix-less copies are in `.gitignore` and must not be committed.
- **`CLAUDE.md`** is required by Claude Code at runtime but is not tracked in git. Run `node setup.js ja` (or `en`) after cloning to generate it.
- **`README.md`** is the sole exception — it is tracked directly (required by GitHub) and is not generated by `setup.js`.
- **`essays/research/*.md`** are single-language research documents — not generated by `setup.js`, tracked normally.
- See the [Framework Development Guide](process-rules/framework-development-en.md) for detailed conventions.
...
| Document                                                                       | Contents                                                           |
| ------------------------------------------------------------------------------ | ------------------------------------------------------------------ |
| [Process Rules](process-rules/full-auto-dev-process-rules-en.md)               | Phase definitions, quality gates, conditional processes            |
...
| [Review Standards](process-rules/review-standards-en.md)                       | R1-R6 review perspectives and checklists                           |
| [Prompt Structure](process-rules/prompt-structure-en.md)                       | S0-S6 agent definition convention                                  |
| [Spec Template](process-rules/spec-template-en.md)                             | ANMS specification template (STFB structure)                       |
| [Glossary](process-rules/glossary-en.md)                                       | Term definitions and rationale                                     |
| [Defect Taxonomy](process-rules/defect-taxonomy-en.md)                         | error / fault / failure / defect / incident causal chain           |
| [Field Issue Handling Rules](process-rules/field-issue-handling-rules-en.md)    | Field testing feedback management (conditional)                    |
