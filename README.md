...
- **`tools/` holds two different kinds of script.** `check-parity`, `check-roster`, `check-links`, `check-tagnames`, `check-setup` and `jsonl2md` exist to guard this repository and run in CI. `gate-guard`, `otel-sink` and `session-meter` run inside a user project, and are the only three that `create.js` ships.
