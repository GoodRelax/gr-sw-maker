...
| 重大度 | 許容される対応 | ゲート要件 |
|--------|-------------|-----------|
| Critical | 修正のみ | フェーズ遷移にはゼロであること（CLAUDE.md 品質目標に準拠） |
| High | 修正のみ | フェーズ遷移にはゼロであること（CLAUDE.md 品質目標に準拠） |
| Medium | 修正 / 据置き / 受容 | 全件に対応記録があること |
| Low | 修正 / 据置き / 受容 | 全件に対応記録があること |

**記録要件:**
- **修正（fix）**: 指摘を修正した。再レビュー報告で解消を確認
- **据置き（defer）**: 指摘を認識するが据置く。`project-records/decisions/` に据置き理由・リスク評価・計画的解消時期を記載した decision 記録を**必ず**作成する
- **受容（accept）**: 指摘を現状のまま受容する。理由をレビュー報告の指摘対応テーブルに記録する

**指摘対応テーブル（review Detail Block 内）:**

| # | 重大度 | 指摘概要 | 対応 | 参照 |
|:-:|:------:|---------|:----:|------|
| 1 | Medium | ... | 修正 | rev 2 で修正 |
| 2 | Medium | ... | 据置き | DEC-003 |
| 3 | Low | ... | 受容 | プロジェクト範囲において許容 |
