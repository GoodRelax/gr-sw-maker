...
Orch エージェントを中心に、16の専門エージェントが役割分担する。モデルも Opus/Sonnet/Haiku の3段階で使い分ける。
...
**競合優位**: プロセス全体の自動化、17エージェント協調、6観点品質ゲート、不具合根本原因分析、用語統制
...
| 観点 | ① OpenSpec | ② Spec Kit | ③ cc-sdd | ④ full-auto-dev |
|------|-----------|-----------|---------|----------------|
| エージェントモデル | 単一エージェント | 単一エージェント | 単一 + サブエージェント | 17エージェント協調 |
| 役割分離 | なし | なし | 基本/サブエージェント | 17役割を明確分離 |
...
8つのプロセスルールブック、17のエージェント定義、22以上のファイルタイプ、条件付きプロセス評価マトリクスの理解が必要であり、導入の初期コストが極めて高い。①の「openspec init → /opsx:propose」や③の「npx cc-sdd」のようなワンコマンドスタートが存在しない。
...
**提案**: 17エージェント・8フェーズの完全構成とは別に、3エージェント・3フェーズの最小構成パッケージを提供する。
