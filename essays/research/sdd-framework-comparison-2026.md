...
Orch エージェントを中心に、17の専門エージェントが役割分担する。モデルも Opus/Sonnet/Haiku の3段階で使い分ける。
...
8つのプロセスルールブック、18のエージェント定義、22以上のファイルタイプ、条件付きプロセス評価マトリクスの理解が必要であり、導入の初期コストが極めて高い。①の「openspec init → /opsx:propose」や③の「npx cc-sdd」のようなワンコマンドスタートが存在しない。
